          ...
          <div id="isi3">
            <table width="100%">
              ...
              <tr><td>Keterangan </td><td>:</td><td>Bahwa orang tersebut adalah benar-benar warga kami yang bertempat tinggal di Dusun <?= unpenetration(ununderscore($data['dusun']))?>, Rt. <?= $data['rt']?>, <?= unpenetration($desa['nama_desa'])?>, <?= unpenetration($desa['nama_kecamatan'])?>, <?= unpenetration($desa['nama_kabupaten'])?> tercatat dalam
                 No. KK: <?= $data['no_kk']?> dengan NIK: <?= $data['nik']?>, kepala keluarga : <?= unpenetration($data['kepala_kk'])?> dan telah menikah di KUA Kecamatan <?= $input['kecamatan_kua']?> pada tanggal <?= tgl_indo(tgl_indo_in($input['tgl_nikah']))?> dengan seseorang yang bernama <?= unpenetration($input['nama_pasangan'])?>.</td></tr>
            </table>
            <table width="100%">
              <tr></tr>
              <tr><td  class="indentasi">Berdasarkan data di atas mohon untuk dibuatkan Duplikat Surat Nikah orang tersebut di atas. </td></tr>
