    ...
    <form id="validasi" action="{{ $form_action }}" method="POST" enctype="multipart/form-data">
        <div id="box-full-screen" class="box box-info">
            <div class="box-header with-border">
                @if ($fullscreen)
                <a id="toggle-btn" href="{{ current_url() }}?fs=0"
                ...
                @else
                <a id="toggle-expand-btn" href="{{ current_url() }}?fs=1"
                    class="btn btn-social bg-navy btn-sm btn-sm visible-xs-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block">
                ...
                <a href="{{ $perbaharui }}"
                    ...
                    title="Perbaharui Data {{ $analisis_master['subjek_nama'] }}"><i class="fa fa-refresh"></i>
                    Pebaharui Data
                    {{ $analisis_master['subjek_nama'] }}</a>
                @endif
                <a href="{{ ci_route('analisis_respon', $analisis_master['id']) }}"
                    class="btn btn-social btn-info btn-sm visible-xs-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block"><i
                        class="fa fa-arrow-circle-left "></i> Kembali Ke Data Sensus</a>
            </div>
            <div class="box-body">
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered table-striped table-hover">
                                    <tr>
                                        ...
                                        <td width="1">:</td>
                                        <td><a
                                                href="{{ ci_route('analisis_master.menu', $analisis_master['id']) }}">{{ $analisis_master['nama'] }}</a>
                                        </td>
                                    </tr>
                                    <tr>
                                ...
                                <table class="table table-bordered dataTable table-hover nowrap">
                                    ...
                                    <tbody>										
                                        ...
                                        <tr>
                                            ...
                                            @if ($analisis_master['id_child'] != 0)
                                            <td nowrap>
                                                <a href="{{ ci_route('analisis_respon.'.$analisis_master['id'].'.child.form.'.$idSubjek, $ang['id']) }}"
                                                    class="btn bg-purple btn-sm" title="Input Data"
                                                    data-remote="false" data-toggle="modal" data-target="#modalBox"
                                                    data-title="{{ $ang['nama'] }} - [{{ $ang['nik'] }}]"><i
                                ...
                                @foreach ($list_jawab as $data)
                                    ...
                                    <tr>
                                        ...
                                        <td class="col-xs-12 col-sm-4 pull-left">
                                            <select class="form-control input-sm select2" name="rb[{{ $data['id'] }}]"
                                                onchange="formAction('mainform', '{{ ci_route('analisis_indikator.'.$analisisMaster['id'].'.kategori') }}')">
                                                <option value=""
                                                    @disabled(($data['referensi'] && $subjek[$data['referensi']]) && $subjek[$data['referensi']] != $data2['kode_jawaban'] || ($data2['cek'] == 1))>
                                                    Pilih Jawaban</option>
                                                @foreach ($data['parameter_respon'] as $data2)
                                                <option
                                                    {{ jecho(($data['referensi'] && $subjek[$data['referensi']]) && $subjek[$data['referensi']] != $data2['kode_jawaban'] || ($data2['cek'] == 1), true, '') }}
                                                    value="{{ $data['id'] }}.{{ $data2['id_parameter'] }}"
                                    ...
                                    <tr>
                                        ...
                                        <td id="op_item">
                                            ...
                                            <div class="checkbox">
                                                <label>
                                                    <input name="cb[{{ $data2['id_parameter'] }}_{{ $data['id'] }}]"
                                                        value="{{ $data['id'] }}.{{ $data2['id_parameter'] }}"
                                                        type="checkbox" @checked($data2['cek'])>
                                    ...
                                    <div class="form-group">
                                        <tr>
                                            ...
                                            <td>
                                                <input
                                                    @readonly($data['referensi'] && $subjek[$data['referensi']])
                                                    class="form-control input-sm" name="ia[{{ $data['id'] }}]"
                                                    value="{{ $data['parameter_respon'][0]['jawaban'] ?? $subjek[$data['referensi']] }}"
                                                    type="number" min=0>
                                            </td>
                                        </tr>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <textarea
                                                    @readonly($data['referensi'] && $subjek[$data['referensi']])
                                                    id="it[{{ $data['id'] }}]" name="it[{{ $data['id'] }}]"
                                                    class="form-control input-sm" style="width:100%"
                                                    rows="5">{{ $data['parameter_respon'][0]['jawaban'] ?? $subjek[$data['referensi']] }}</textarea>
                                            </td>
                                        </tr>
                                    </div>
                                    @endif
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-2">
                                        <input type="hidden" name="old_file"
                                            value="{{ $list_bukti[0]['pengesahan'] }}">
                                        <img class="attachment-img img-responsive"
                                            src="{{ base_url(LOKASI_PENGESAHAN . $list_bukti[0]['pengesahan']) }}"
                                            alt="Bukti Pengesahan">
                                    </div>
                                </div>
                                @endif
                                <div class="form-group">
