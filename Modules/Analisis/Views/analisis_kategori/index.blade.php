	...
	<div class="col-md-8 col-lg-9">
		...
		<div class="box box-info">
			<div class="box-header with-border">
				@if (can('u'))
				<a href="{{ ci_route('analisis_kategori.'.$analisis_master['id'].'.form' ) }}"
					data-remote="false" data-toggle="modal" data-target="#modalBox" data-title="Tambah Data"
					class="btn btn-social btn-success btn-sm visible-xs-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block"><i class='fa fa-plus'></i>
					Tambah</a>

				@endif
				@if (can('h'))
				<a href="#confirm-delete" title="Hapus Data"
					onclick="deleteAllBox('mainform', '{{ ci_route('analisis_kategori.'.$analisis_master['id'].'.delete') }}')"
...
<script>
    $(document).ready(function () {
        var TableData = $('#tabeldata').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: "{{ ci_route('analisis_kategori.'.$analisis_master['id'].'.datatables') }}",
            columns: [{
                    data: 'ceklist',
                    class: 'padat',
                    searchable: false,
