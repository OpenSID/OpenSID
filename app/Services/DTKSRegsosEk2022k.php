...
class DTKSRegsosEk2022k
{
    ...
    public function info()
    {
        ...
        foreach($relasi_program as $form_input_name => $item){
            ...
            else if($pengaturan_program && substr($form_input_name, -(strlen('default'))) === 'default'){
    ...
    protected function generateDefaultDtks($dtks) : Dtks
    {
        ...
        $dtks = $this->syncKepesertaanProgramKeluarga($dtks);
    ...
    protected function savePengaturanProgram($request)
    {
        ...
        foreach($relasi as $form_input_name => $item){
            ...
            else if($request[$form_input_name] != '' && $pengaturan_program && $request[$form_input_name] != $pengaturan_program->id_bantuan){
                $pengaturan_program->update(['id_bantuan' => $request[$form_input_name]]);
            }
            else if($request[$form_input_name] != '' && ! $pengaturan_program && (substr($key, -(strlen('default'))) !== 'default')){
                $to_be_inserted[] = [
                    'versi_kuisioner' => '2',
    ...
    public function syncKetDemografi(DtksAnggota $dtks_anggota, $agt, Penduduk $kepala_keluarga, $ref_eloquent_collection) : DtksAnggota
    {
        ...
        $is_ibu_anak_punya_data_kia = $ref_eloquent_collection['kia']->filter(function($item) use($agt){
    ...
    public function syncPendidikan(DtksAnggota $dtks_anggota, $agt, Collection $daftar_pendidikan) : DtksAnggota
    {
        ...
        // tidak sekolah lagi
        else if($agt->pendidikan_sedang_id == 18){
            $dtks_anggota->kd_partisipasi_sekolah  = 3; // 413
        }
        // sedang sekolah
        else if(strpos($nama_pendidikan, 'SEDANG ') == 0){
    ...
    public function syncProgramPerlindunganSosial(DtksAnggota $dtks_anggota, $agt, Penduduk $kepala_keluarga, $ref_eloquent_collection) : DtksAnggota
    {
        ...
        if($pengaturan_programs->count() > 0){
            ...
            $pengaturan_program_selain_default = $pengaturan_programs->filter(function($item){
                return substr($item->kode, -(strlen('default'))) !== 'default';
            });
            $pengaturan_program_default = $pengaturan_programs->filter(function($item){
                return substr($item->kode, -(strlen('default'))) === 'default';
            })->keyBy('target_field');
            ...
            foreach($pengaturan_program_selain_default as $item){
                ...
                if($kepesertaan_anggota_ini && $kepesertaannya <= 1){
                    ...
                    else if(in_array($item->kode, array_keys($nilai_jaminan_ketenagakerjaan))){
                        $to_be_updated[$fields[0]]  += $to_be_updated[$fields[0]]
    ...
    protected function saveRelatedAttribute($dtks_or_dtks_anggota)
    {
        ...
        }else if($dtks_or_dtks_anggota instanceof DtksAnggota){
    ...
    {
        $c = collect($daftar_pilihan)->search(function($item, $key) use($search_value){
