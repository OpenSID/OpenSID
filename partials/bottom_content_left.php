										...
										<div class="post_commentbox">
											<span class="meta_date"><?= tgl_indo2($data['tgl_upload']);?>&nbsp;
												...
												<i class="fa fa-comments"></i>
													<?php $baca_komentar = $this->db->query("SELECT * FROM komentar WHERE id_artikel = '".$data['id']."'"); $komentarku = $baca_komentar->num_rows();
												echo number_format($komentarku,0,',','.')." Komentar"; ?>&nbsp;
												<?php if (trim($data['kategori']) != '') : ?>
													<a href="<?= site_url('first/kategori/'.$data['kat_slug']) ?>"><i class='fa fa-tag'></i><?= $data['kategori']?></a>&nbsp;
												<?php endif; ?>
											</span>
										</div>
