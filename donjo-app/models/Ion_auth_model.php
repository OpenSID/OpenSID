...
class Ion_auth_model extends CI_Model
{
	...
	public function user_gp_privilegs($gp_id)
	{
		$this->db->select('*');
		$this->db->from('groups');
		$this->db->join('group_perm', 'group_perm.group_id = groups.id');
		$this->db->join('setting_modul', 'setting_modul.id = group_perm.perm_id');
		$this->db->where('group_perm.group_id',$gp_id);
		$query = $this->db->get();
		return $query->result();
	}

        public function user_gp_privilegs_create($gp_id)
	{
		$this->db->select('*');
		$this->db->from('groups');
		$this->db->join('group_perm', 'group_perm.group_id = groups.id');
		$this->db->join('setting_modul', 'setting_modul.id = group_perm.create_id');
		$this->db->where('group_perm.group_id',$gp_id);
		$query = $this->db->get();
		return $query->result();
	}

        public function user_gp_privilegs_update($gp_id)
	{
		$this->db->select('*');
		$this->db->from('groups');
		$this->db->join('group_perm', 'group_perm.group_id = groups.id');
		$this->db->join('setting_modul', 'setting_modul.id = group_perm.update_id');
		$this->db->where('group_perm.group_id',$gp_id);
		$query = $this->db->get();
		return $query->result();
	}

       public function user_gp_privilegs_delete($gp_id)
	{
		$this->db->select('*');
		$this->db->from('groups');
		$this->db->join('group_perm', 'group_perm.group_id = groups.id');
		$this->db->join('setting_modul', 'setting_modul.id = group_perm.delete_id');
		$this->db->where('group_perm.group_id',$gp_id);
		$query = $this->db->get();
		return $query->result();
	}

        public function user_gp_privilegs_print($gp_id)
	{
		$this->db->select('*');
		$this->db->from('groups');
		$this->db->join('group_perm', 'group_perm.group_id = groups.id');
		$this->db->join('setting_modul', 'setting_modul.id = group_perm.print_id');
		$this->db->where('group_perm.group_id',$gp_id);
		$query = $this->db->get();
		return $query->result();
	}
