<?php class Export_model extends CI_Model {
  ...
  // Export data penduduk ke format Import Excel
	public function export_excel($opendk = FALSE)
	{
		$selectedColumn = 'k.alamat, c.dusun, c.rw, c.rt, p.nama, k.no_kk, p.nik, p.sex, p.tempatlahir, p.tanggallahir, p.agama_id, p.pendidikan_kk_id, p.pendidikan_sedang_id, p.pekerjaan_id, p.status_kawin, p.kk_level, p.warganegara_id, p.nama_ayah, p.nama_ibu, p.golongan_darah_id, p.akta_lahir, p.dokumen_pasport, p.tanggal_akhir_paspor, p.dokumen_kitas, p.ayah_nik, p.ibu_nik, p.akta_perkawinan, p.tanggalperkawinan, p.akta_perceraian, p.tanggalperceraian, p.cacat_id, p.cara_kb_id, p.hamil, p.ktp_el, p.status_rekam, p.alamat_sekarang';
		if($opendk){
			$selectedColumn .= ', p.id, p.status_dasar, p.created_at, p.updated_at';
		}
