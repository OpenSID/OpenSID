<?php class analisis_respon_rtm_model extends CI_Model{
	...
	function autocomplete(){
		$sql   = "SELECT no_kk FROM tweb_rtm
		UNION SELECT t.nama FROM tweb_rtm u LEFT JOIN tweb_penduduk t ON u.nik_kepala = t.id LEFT JOIN tweb_wil_clusterdesa c ON t.id_cluster = c.id WHERE 1  ";
	...
	function paging($p=1,$o=0){
		...
		$sql      = "SELECT COUNT(u.id) AS id FROM tweb_rtm u  LEFT JOIN tweb_penduduk p ON u.nik_kepala = p.id WHERE 1";
		$sql     .= $this->search_sql(); 
		$query    = $this->db->query($sql);
		$row      = $query->row_array();
		$jml_data = $row['id'];
	...
	function get_subjek($id=0){
		$sql   = "SELECT u.*,p.nama FROM tweb_rtm u LEFT JOIN tweb_penduduk p ON u.id = p.id_rtm WHERE p.rtm_level = 1 AND u.id=?";
		$query = $this->db->query($sql,$id);
		return $query->row_array();
	}	
