<?php class analisis_respon_kelompok_model extends CI_Model{
	...
	function autocomplete(){
		...
		UNION SELECT t.nama FROM kelompok u LEFT JOIN tweb_penduduk t ON u.id_ketua = t.id LEFT JOIN tweb_wil_clusterdesa c ON t.id_cluster = c.id WHERE 1  ";
	...
	function search_sql(){
		...
		$cari = $_SESSION['cari'];
			...
			$kw = '%' .$kw. '%';
			$search_sql= " AND (u.nama LIKE '$kw' OR p.nama LIKE '$kw')";
			return $search_sql;
			}
		}
	...
	function get_subjek($id=0){
		$sql   = "SELECT u.* FROM kelompok u  WHERE u.id=?";
		$query = $this->db->query($sql,$id);
		return $query->row_array();
	}	
