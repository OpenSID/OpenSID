...
define("KOLOM_IMPOR_KELUARGA", serialize(array(
	...
	"ktp_el" => "34",
	"status_rekam" => "35",
	"telepon" => "36", // <--- Tambah Telepon
	"foto" => "37", // <--- Tambah foto untuk memungkinkan Upload foto dari ftp atau langsung ke folder
)));
// <--- Tambah Telepon
...
*/
	...
	protected function data_import_valid($isi_baris)
	{
		...
		//Menambah validasi Telepon alias nomor HP -- kode dibawah masih memuculkan error import jika di uncomment
		//if ($isi_baris['telepon'] != "" and !($isi_baris['telepon'] >= 9 && $isi_baris['telepon'] <= 12)) return 'kode telepon tidak dikenal';
	...
	private function get_isi_baris($data, $i)
	{
		...
		// Nambahin Nomor Telepon
		$isi_baris['telepon'] = trim($data->val($i, $kolom_impor_keluarga['telepon']));
		$isi_baris['foto'] = trim($data->val($i, $kolom_impor_keluarga['foto']));
		return $isi_baris;
	}
