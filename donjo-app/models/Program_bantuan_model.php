...
class Program_bantuan_model extends MY_Model
{
    ...
    public function add_peserta($program_id)
    {
        ...
        if ($_FILES['file']['name']) {
            $data['kartu_peserta'] = unggah_file(['upload_path' => LOKASI_DOKUMEN, 'allowed_types' => GAMBAR_ALLOWED_TYPES]);
        }
    ...
    public function edit_peserta($id)
    {
        ...
        if ($_FILES['file']['name']) {
            $peserta->kartu_peserta = unggah_file(['upload_path' => LOKASI_DOKUMEN, 'allowed_types' => GAMBAR_ALLOWED_TYPES], $peserta->kartu_peserta);
        } else {
