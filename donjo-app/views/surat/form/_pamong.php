<div class="form-group">
	...
	<div class="col-sm-6">
		<select class="select2 form-control required input-sm" id="pamong" name="pamong" <?= $disabled ?> onchange="var nip=$(this).find(':selected').data('nip'); $('#pamong_nip').val(nip);$(this).closest('.box-body').find('select[name=jabatan]').val($(this).find(':selected').data('jabatan')) ">
			...
			<?php foreach ($pamong AS $data):?>
				...
				<option value="<?= $data['pamong_nama']?>" data-jabatan="<?= trim($data['jabatan']) ?>" <?php if ($data['pamong_ttd']==1): $pamong_nip =	$data['pamong_nip'];?>selected <?php endif;?> data-nip="<?= $data['pamong_nip']?>">
					<?= $data['pamong_nama']?> (<?= $data['jabatan']?>) <?php if (!empty($tmp_nip)) echo "NIP: ".$data['pamong_nip']?>
				</option>
			<?php endforeach;?>
		</select>
...
<div class="form-group">
	...
	<div class="col-sm-6">
		<select class="form-control input-sm required" id="jabatan" name="jabatan" <?=  $disabled ?>>
