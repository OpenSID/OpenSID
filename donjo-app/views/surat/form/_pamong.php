	...
	<td>
		<select name="pamong" class="inputbox required" onchange="var nip=$(this).find(':selected').data('nip'); $('#pamong_nip').val(nip);$(this).closest('table').find('select[name=jabatan]').val($(this).find(':selected').data('jabatan')) ">
			...
			<?php foreach($pamong AS $data){?>
				...
				<option value="<?php echo $data['pamong_nama']?>" data-jabatan="<?php echo unpenetration(trim($data['jabatan'])) ?>" <?php if($data['pamong_ttd']==1) { $pamong_nip = $data['pamong_nip']; echo "selected"; }?> data-nip="<?php echo $data['pamong_nip']?>">
...
<tr>
	...
	<td>
		<select name="jabatan"  class="inputbox required">
			...
			<?php foreach($pamong AS $data){?>
				<option <?php if($data['pamong_ttd']==1) echo "selected"?>><?php echo unpenetration(trim($data['jabatan'])) ?></option>
			<?php }?>
		</select>
	</td>
</tr>
