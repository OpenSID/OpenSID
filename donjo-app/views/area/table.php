		...
		<form class="form-inline" id="mainform" name="mainform" action="" method="post">
          ...
          <?php $this->load->view('plan/nav.php')?>
            ...
            <div class="card-header with-border">
							<a href="<?= site_url("area/form")?>" class="btn btn-flat btn-success btn-xs btn-xs visible-xs-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block visible-xl-inline-block text-left"  title="Tambah Data Baru">
								<i class="fa fa-plus"></i>Tambah Data Baru
            	</a>
							<?php if ($this->CI->cek_hak_akses('h')): ?>
								<a href="#confirm-delete" title="Hapus Data" onclick="deleteAllBox('mainform', '<?= site_url("area/delete_all/$p/$o")?>')" class="btn btn-flat btn-danger btn-xs visible-xs-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block visible-xl-inline-block text-left hapus-terpilih"><i class='fa fa-trash-o'></i> Hapus Data Terpilih</a>
							<?php endif; ?>
								...
								<div class="col-sm-12">
									<div class="dataTables_wrapper dt-bootstrap no-footer">
										<form class="form-inline" id="mainform" name="mainform" action="" method="post">
											<div class="row">
												<div class="col-sm-7">
													<select class="form-control form-control-sm" name="filter" onchange="formAction('mainform', '<?= site_url('area/filter')?>')">
														<option value="">Semua</option>
														<option value="1" <?php if ($filter==1): ?>selected<?php endif ?>>Aktif</option>
														<option value="2" <?php if ($filter==2): ?>selected<?php endif ?>>Tidak Aktif</option>
													</select>
													<select class="form-control form-control-sm" name="polygon" onchange="formAction('mainform', '<?= site_url('area/polygon')?>')">
														<option value="">Kategori</option>
														<?php foreach ($list_polygon AS $data): ?>
															<option value="<?= $data['id']?>" <?php if ($polygon == $data['id']): ?>selected<?php endif ?>><?= $data['nama']?></option>
														<?php endforeach;?>
													</select>
													<select class="form-control form-control-sm" name="subpolygon" onchange="formAction('mainform', '<?= site_url('area/subpolygon')?>')">
															...
															<tbody>
																<?php foreach ($main as $data): ?>
																	<tr>
																		...
																		<td><?=$data['no']?></td>
																		<td nowrap>
																			<a href="<?= site_url("area/form/$p/$o/$data[id]")?>" class="btn btn-warning btn-flat btn-xs"  title="Ubah"><i class="fa fa-edit"></i></a>
																			<a href="<?= site_url("area/ajax_area_maps/$p/$o/$data[id]")?>" class="btn bg-olive btn-flat btn-xs" title="Lokasi <?= $data['nama']?>"><i class="fa fa-map"></i></a>
																			<?php if ($data['enabled'] == '2'): ?>
																				<a href="<?= site_url('area/area_lock/'.$data['id'])?>" class="btn bg-navy btn-flat btn-xs" title="Aktifkan"><i class="fa fa-lock">&nbsp;</i></a>
																			<?php elseif ($data['enabled'] == '1'): ?>
																				<a href="<?= site_url('area/area_unlock/'.$data['id'])?>" class="btn bg-navy btn-flat btn-xs" title="Non Aktifkan"><i class="fa fa-unlock"></i></a>
																			<?php endif ?>
																			<a href="#" data-href="<?= site_url("area/delete/$p/$o/$data[id]")?>" class="btn bg-maroon btn-flat btn-xs"  title="Hapus" data-toggle="modal" data-target="#confirm-delete"><i class="fa fa-trash-o"></i></a>
																	  </td>
