...
<td>
<a href="<?=site_url("point/form/$p/$o/$data[id]")?>" class="ui-icons icon-edit tipsy south" title="Edit Data"></a><a href="<?=site_url("point/delete/$p/$o/$data[id]")?>" class="ui-icons icon-remove tipsy south" title="Delete Data" target="confirm" message="Apakah Anda Yakin?" header="Hapus Data"></a><?if($data['enabled'] == '2'):?><a href="<?=site_url('point/point_lock/'.$data['id'])?>" class="ui-icons icon-lock tipsy south" title="Enable point"></a><?elseif($data['enabled'] == '1'): ?><a href="<?=site_url('point/point_unlock/'.$data['id'])?>" class="ui-icons icon-unlock tipsy south" title="Disable point"></a><a href="<?=site_url("point/sub_point/$data[id]")?>" class="ui-icons icon-document-table tipsy south" title="Rincian Sub point"></a><a href="<?=site_url("point/ajax_add_sub_point/$data[id]")?>" target="ajax-modal" rel="window" header="Tambah Sub point <?=$data['nama']?>" class="ui-icons icon-plus tipsy south" title="Tambah Sub point"></a>
<?endif?>
</td>
...
<a href="<?=site_url("point/index/$i/$o")?>" <? jecho($p,$i,"class='uibutton special'")?> class="uibutton"><?=$i?></a>
<? endfor; ?>
</div>
<div class="uibutton-group">
<? if($paging->next): ?>
