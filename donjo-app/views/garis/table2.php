...
<button type="button" title="Delete Data" onclick="deleteAllBox('mainform','<?php  echo site_url("garis/delete_all/$p/$o")?>')" class="uibutton tipsy south"><span class="fa fa-trash">&nbsp;</span>Delete Data
</div>
</div>
</div>
...
<div class="left">
		...
		<select name="filter" onchange="formAction('mainform','<?php  echo site_url('garis/filter')?>')">
		...
		<select name="line" onchange="formAction('mainform','<?php  echo site_url('garis/line')?>')">
		...
		<select name="subline" onchange="formAction('mainform','<?php  echo site_url('garis/subline')?>')">
...
<button type="button" onclick="$('#'+'mainform').attr('action','<?php  echo site_url('garis/search')?>');$('#'+'mainform').submit();" class="uibutton tipsy south"title="Cari Data"><span class="fa fa-search">&nbsp;</span>Search</button>
</div>
</div>
<table class="list">
<thead>
...
<td>
<a href="<?php  echo site_url("garis/form/$p/$o/$data[id]")?>" class="fa fa-edit" title="Edit Data"></a><a href="<?php  echo site_url("garis/delete/$p/$o/$data[id]")?>" class="fa fa-remove" title="Delete Data" target="confirm" message="Apakah Anda Yakin?" header="Hapus Data"></a><?php  /*if($data['enabled'] == '2'):?><a href="<?php  echo site_url('garis/garis_lock/'.$data['id'])?>" class="ui-icons fa fa-lock tipsy south" title="Enable garis"></a><?php  elseif($data['enabled'] == '1'): ?><a href="<?php  echo site_url('garis/garis_unlock/'.$data['id'])?>" class="ui-icons fa fa-unlock tipsy south" title="Disable garis"></a>*/?><a href="<?php  echo site_url("garis/ajax_garis_maps/$p/$o/$data[id]")?>" target="ajax-modalz" rel="window" header="garis <?php  echo $data['nama']?>" class="fa fa-map" title="garis <?php  echo $data['nama']?>"></a>
...
<form id="paging" action="<?php  echo site_url('garis')?>" method="post">
