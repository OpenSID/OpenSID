...
<div class="lmenu">
<ul>
<li <?if($tip==1)echo "class='selected'";?>><a href="<?=site_url("garis/index/1")?>">Atas</a></li>
<li <?if($tip==2)echo "class='selected'";?>><a href="<?=site_url("garis/index/2")?>">Atas Kiri</a></li>
...
<div class="left">
		...
		<select name="filter" onchange="formAction('mainform','<?=site_url('garis/filter')?>')">
			<option value="">Semua</option>
			<option value="1" <?if($filter==1) :?>selected<?endif?>>Enabled</option>
			<option value="2" <?if($filter==2) :?>selected<?endif?>>Disabled</option>
		</select>

		<select name="line" onchange="formAction('mainform','<?=site_url('garis/line')?>')">
			<option value="">Kategori</option>
			<?foreach($list_line AS $data){?>
			<option value="<?=$data['id']?>" <?if($line == $data['id']) :?>selected<?endif?>><?=$data['nama']?></option>
			<?}?>
		</select>
			
		<select name="subline" onchange="formAction('mainform','<?=site_url('garis/subline')?>')">
			<option value="">Jenis</option>
			<?foreach($list_subline AS $data){?>
			<option value="<?=$data['id']?>" <?if($subline == $data['id']) :?>selected<?endif?>><?=$data['nama']?></option>
			<?}?>
		</select>
...
<button type="button" onclick="$('#'+'mainform').attr('action','<?=site_url('garis/search')?>');$('#'+'mainform').submit();" class="uibutton tipsy south"title="Cari Data"><span class="ui-icon ui-icon-search">&nbsp;</span>Search</button>
</div>
</div>
<table class="list">
<thead>
...
<td>
<a href="<?=site_url("garis/form/$p/$o/$data[id]")?>" class="ui-icons icon-edit tipsy south" title="Edit Data"></a><a href="<?=site_url("garis/delete/$p/$o/$data[id]")?>" class="ui-icons icon-remove tipsy south" title="Delete Data" target="confirm" message="Apakah Anda Yakin?" header="Hapus Data"></a><?/*if($data['enabled'] == '2'):?><a href="<?=site_url('garis/garis_lock/'.$data['id'])?>" class="ui-icons icon-lock tipsy south" title="Enable garis"></a><?elseif($data['enabled'] == '1'): ?><a href="<?=site_url('garis/garis_unlock/'.$data['id'])?>" class="ui-icons icon-unlock tipsy south" title="Disable garis"></a>*/?><a href="<?=site_url("garis/ajax_garis_maps/$p/$o/$data[id]")?>" target="ajax-modalz" rel="window" header="garis <?=$data['nama']?>" class="ui-icons icon-maps tipsy south" title="garis <?=$data['nama']?>"></a>
...
<form id="paging" action="<?=site_url('garis')?>" method="post">
...
<a href="<?=site_url("garis/index/$i/$o")?>" <? jecho($p,$i,"class='uibutton special'")?> class="uibutton"><?=$i?></a>
<? endfor; ?>
</div>
<div class="uibutton-group">
<? if($paging->next): ?>
