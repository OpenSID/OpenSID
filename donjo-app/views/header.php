...
<body id="sidebar_collapse" class="<?= $this->setting->warna_tema_admin ?> sidebar-mini fixed">
    <div class="wrapper">
        ...
        <?php if ($kategori && $kategori !== 'pelanggan' && can('u', $this->akses_modul ?? ($this->modul_ini ?? $this->sub_modul_ini))) : ?>
            <div class="modal fade" id="pengaturan" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                            <h4 class="modal-title" id="myModalLabel"> Pengaturan <?= ucwords(str_replace('_', ' ', $kategori)) ?></h4>
                        </div>
                        <?php $this->load->view('global/modal_setting', ['kategori' => [$kategori]]) ?>
                    </div>
                </div>
            </div>
        <?php endif ?>
