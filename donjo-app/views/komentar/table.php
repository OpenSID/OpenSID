			...
			<div class="row mb-2">
				...
				<div class="col-sm-6">
					<ol class="breadcrumb float-sm-right">
						...
						<li class="breadcrumb-item active">Pengaturan Komentar</li>
					</ol>
				</div>
			</div>
		</div>
								...
								<div class="dataTables_wrapper dt-bootstrap no-footer">
									<form class="form-inline" id="mainform" name="mainform" action="" method="post">
										...
										<div class="container-fluid">
											<div class="row mb-2">
												<div class="col-sm-6">
													<select class="form-control form-control-sm " name="filter" onchange="formAction('mainform', '<?=site_url('komentar/filter')?>')">
												...
												<tbody>
													<?php foreach ($main as $data): ?>
														<tr>
															...
															<td><?=$data['no']?></td>
															<td nowrap>
																<a href="<?= site_url("komentar/form/$p/$o/$data[id]")?>" class="btn btn-warning btn-flat btn-xs" title="Ubah"><i class="fa fa-edit"></i></a>
																<?php if ($data['status'] == '2'): ?>
																	<a href="<?= site_url('komentar/komentar_lock/'.$data['id'])?>" class="btn bg-navy btn-flat btn-xs" title="Aktifkan Komentar"><i class="fa fa-lock">&nbsp;</i></a>
																<?php elseif ($data['status'] == '1'): ?>
																	<a href="<?= site_url('komentar/komentar_unlock/'.$data['id'])?>" class="btn bg-navy btn-flat btn-xs" title="Non Aktifkan Komentar"><i class="fa fa-unlock"></i></a>
																<?php endif ?>
																<a href="#" data-href="<?= site_url("komentar/delete/$p/$o/$data[id]")?>" class="btn bg-maroon btn-flat btn-xs" title="Hapus" data-toggle="modal" data-target="#confirm-delete"><i class="fa fa-trash-o"></i></a>
															</td>
											...
											<div class="dataTables_paginate paging_simple_numbers">
												<ul class="pagination">
													<?php if ($paging->start_link): ?>
														<li class="page-item disabled"><a class="page-link" href="<?=site_url("komentar/index/$paging->start_link/$o")?>" aria-label="First"><span aria-hidden="true">Awal</span></a></li>
													<?php endif; ?>
													<?php if ($paging->prev): ?>
														<li class="page-item"><a class="page-link" href="<?=site_url("komentar/index//$paging->prev/$o")?>" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>
													<?php endif; ?>
													<?php for ($i=$paging->start_link;$i<=$paging->end_link;$i++): ?>
														<li <?=jecho($p, $i, "class='page-item active'")?>><a class="page-link" href="<?= site_url("komentar/index/$i/$o")?>"><?= $i?></a></li>
													<?php endfor; ?>
													<?php if ($paging->next): ?>
														<li class="page-item"><a class="page-link" href="<?=site_url("komentar/index/$paging->next/$o")?>" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>
													<?php endif; ?>
													<?php if ($paging->end_link): ?>
														<li class="page-item disabled"><a class="page-link" href="<?=site_url("komentar/index/$paging->end_link/$o")?>" aria-label="Last"><span aria-hidden="true">Akhir</span></a></li>
													<?php endif; ?>
												</ul>
											</div>
										</div>
