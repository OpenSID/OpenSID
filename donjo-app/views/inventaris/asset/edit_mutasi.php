						...
						<form class="form-horizontal" id="update_mutasi_asset" name="update_mutasi_asset" method="post" action="">
							...
							<div class="col-md-6">
								<div class="form-group">
									...
									<div class="col-sm-9">
										<input maxlength="50" class="form-control" name="tahun" id="tahun" value="<?= $main->tahun_pengadaan;?>" disabled/>
									</div>
								</div>
								<div class="form-group">
...
<script>
	...
	$("#update_mutasi_asset").validate(
	{
		submitHandler: function(form)
		{
			...
			$.ajax(
			{
				url: '<?= site_url("api_inventaris_asset/update_mutasi"); ?>'+'/'+ $('#id').val(),
				method: 'post',
				dataType: 'json',
				data: formInput,
				contentType: false,
				...
				success: function()
				{
					...
					setTimeout(function()
					{
						window.location.href = '<?= site_url("inventaris_asset/mutasi"); ?>';
					}, 2000)
				},
				error: function(err)
				{
