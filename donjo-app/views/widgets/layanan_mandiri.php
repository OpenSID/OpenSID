...
<?php
		...
		if($this->session->mandiri_wait == 1) { ?>
			<div class="box box-primary box-solid">
				<div class="box-header">
					...
					Silakan datang atau hubungi operator <?= $this->setting->sebutan_desa?> untuk mendapatkan kode PIN anda.
				</div>
				<div class="box-body">
		...
		<?php } else { ?>
			<div class="box box-primary box-solid">
				...
				<div class="box-body">
					...
					<form action="<?= site_url('mandiri_login/auth')?>" method="post">
					...
					<button type="submit" id="but" style="margin-left:0px">Masuk</button>
						<?php if($this->session->mandiri_try AND isset($this->session->mandiri) AND $this->session->mandiri == -1) { ?>
						<div id="note">
							Kesempatan mencoba <?= ($this->session->mandiri_try-1); ?> kali lagi.
						</div>
						<?php }?>
						<?php if(isset($this->session->mandiri) AND $this->session->mandiri==-1) { ?>
						<div id="note">
							Login Gagal. Username atau Password yang Anda masukkan salah!
						</div>
						<?php }?>
	...
	?>
		<div class="box box-primary box-solid">
			...
			<div class="box-body">
				...
				<table id="mandiri" width="100%" border="0" cellspacing="0" cellpadding="0">
					...
					<tr>
						<td colspan="3"><h4><a href="<?= site_url('mandiri_web/logout'); ?>" class=""><button type="button" class="btn btn-danger btn-block">KELUAR</button></a></h4></td>
					</tr>
				</table>
			</div>
		</div>
		<?php if(isset($this->session->lg) AND $this->session->lg == 1) { ?>
			<div class="box box-primary box-solid">
				<div class="box-header">
					<h3 class="box-title"><i class="fa fa-user"></i> Layanan Mandiri</h3><br />
		...
		<?php } else if(isset($this->session->lg) AND $this->session->lg == 2) { ?>
			<div class="box box-primary box-solid">
				<div class="box-header">
					<h3 class="box-title"><i class="fa fa-user"></i> Layanan Mandiri</h3><br />
					Untuk keamanan silahkan ubah kode PIN Anda.
