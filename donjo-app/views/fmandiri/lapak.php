										...
										<?php for ($i = 0; $i < $this->setting->banyak_foto_tiap_produk; $i++) : ?>
											<?php if ($foto[$i]) : ?>
												<div class="item <?= jecho($i, 0, 'active'); ?>">
													...
													<?php else : ?>
														<img class="card-img-top" style="width: auto; max-height: 170px;" src="<?= assets('images/404-image-not-found.jpg') ?>" alt="Foto Produk" />
													<?php endif; ?>
												</div>
											<?php endif; ?>
										<?php endfor; ?>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$(document).on('shown.bs.modal', '#map-modal', function(event) {
			...
			let logo = L.icon({
				iconUrl: "<?= assets('images/gis/point/fastfood.png'); ?>",
			});
