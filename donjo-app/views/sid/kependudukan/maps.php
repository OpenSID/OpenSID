...
(function() {
		...
		<?if($penduduk['lat'] != ""){?>
          ...
          zoom: <?=$penduduk['zoom']?>, // from 0 to 23
          mapTypeId: google.maps.MapTypeId.<?=strtoupper($penduduk['map_tipe'])?> // ROADMAP, TERRAIN, SATELLITE,HYBRID
		  <?}else{
			  if($desa['lat']!=""){?>
   ...
   // map = new google.maps.Map(document.getElementById('map'), options);
		var marker = new google.maps.Marker({<?if($penduduk['lat'] != ""){?>
