		...
		<div class="card card-outline card-info">
			<div class="card-header with-border">
				<div class="btn-group btn-group-vertical">
					<a class="btn btn-flat btn-success btn-xs" data-toggle="dropdown"><i class='fa fa-plus'></i> Tambah KK Baru</a>
					<ul class="dropdown-menu text-xs" role="menu">
						<li>
							<a href="<?= site_url('keluarga/form')?>" class="btn btn-flat btn-block btn-xs" title="Tambah Data KK Baru"><i class="fa fa-plus"></i> Tambah Penduduk Baru</a>
						</li>
						<li>
							<a href="<?= site_url('keluarga/form_old')?>" class="btn btn-flat btn-block btn-xs" title="Tambah Data KK dari keluarga yang sudah ter-input" data-remote="false" data-toggle="modal" data-target="#modalBox" data-title="Tambah Data Kepala Keluarga"><i class="fa fa-plus"></i> Dari Penduduk Sudah Ada</a>
						</li>
					</ul>
				</div>
				<a href="<?= site_url("keluarga/ajax_cetak/$o/cetak")?>" class="btn btn-flat bg-purple btn-xs visible-xs-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block visible-xl-inline-block text-left" title="Cetak Data"  data-remote="false" data-toggle="modal" data-target="#modalBox" data-title="Cetak Data" target="_blank"><i class="fa fa-print"></i> Cetak</a>
				<a href="<?= site_url("keluarga/ajax_cetak/$o/unduh")?>" class="btn btn-flat bg-navy btn-xs visible-xs-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block visible-xl-inline-block text-left" title="Unduh Data" data-remote="false" data-toggle="modal" data-target="#modalBox" data-title="Unduh Data" target="_blank"><i class="fa fa-download"></i> Unduh</a>
				<div class="btn-group btn-group-vertical">
					<a class="btn btn-flat bg-maroon btn-xs" data-toggle="dropdown"><i class='fa fa-arrow-circle-down'></i> Aksi Data Terpilih</a>
					<ul class="dropdown-menu text-xs" role="menu">
						<li>
							<a href="" class="btn btn-flat btn-block btn-xs aksi-terpilih" title="Cetak Kartu Keluarga" onclick="formAction('mainform','<?= site_url("keluarga/cetak_kk_all")?>', '_blank'); return false;"><i class="fa fa-print"></i> Cetak Kartu Keluarga</a>
						</li>
						<li>
							<a href="" class="btn btn-flat btn-block btn-xs aksi-terpilih" title="Unduh Kartu Keluarga" onclick="formAction('mainform','<?= site_url("keluarga/doc_kk_all")?>'); return false;"><i class="fa fa-download"></i> Unduh Kartu Keluarga</a>
						</li>
						<?php if ($this->CI->cek_hak_akses('h')): ?>
							<li>
								<a href="#confirm-delete" class="btn btn-flat btn-block btn-xs hapus-terpilih" title="Hapus Data" onclick="deleteAllBox('mainform', '<?=site_url("keluarga/delete_all")?>')"><i class="fa fa-trash-o"></i> Hapus Data Terpilih</a>
							</li>
						<?php endif; ?>
					</ul>
				</div>
				<div class="btn-group-vertical">
					<a class="btn btn-flat btn-info btn-xs" data-toggle="dropdown"><i class='fa fa-arrow-circle-down'></i> Pilih Aksi Lainnya</a>
					<ul class="dropdown-menu text-xs" role="menu">
						<li>
							<a href="<?= site_url("keluarga/search_kumpulan_kk")?>" class="btn btn-flat btn-block btn-xs" title="Pilihan Kumpulan KK" data-remote="false" data-toggle="modal" data-target="#modalBox" data-title="Pilihan Kumpulan KK"><i class="fa fa-search"></i> Pilihan Kumpulan KK</a>
						</li>
					</ul>
				</div>
						...
						<div class="container-fluid">
							<div class="row mb-2">
								<div class="col-sm-9">
									...
									<select class="form-control form-control-sm " name="dusun" onchange="formAction('mainform','<?= site_url('keluarga/dusun')?>')">
									...
									<?php if ($dusun): ?>
										<select class="form-control form-control-sm" name="rw" onchange="formAction('mainform','<?= site_url('keluarga/rw')?>')" >
									...
									<?php endif; ?>
									<?php if ($rw): ?>
										<select class="form-control form-control-sm" name="rt" onchange="formAction('mainform','<?= site_url('keluarga/rt')?>')">
								...
								<tbody>
									<?php foreach ($main as $data): ?>
										<tr>
											...
											<td class="aksi">
												<a href="<?= site_url("keluarga/anggota/$p/$o/$data[id]")?>" class="btn bg-purple btn-flat btn-xs" title="Rincian Anggota Keluarga (KK)"><i class="fa fa-list-ol"></i></a>
												<a href="<?= site_url("keluarga/form_a/$p/$o/$data[id]")?>" class="btn btn-success btn-flat btn-xs " title="Tambah Anggota Keluarga" ><i class="fa fa-plus"></i> </a>
												<a href="<?= site_url("keluarga/edit_nokk/$p/$o/$data[id]")?>" title="Ubah Data" data-remote="false" data-toggle="modal" data-target="#modalBox" data-title="Ubah Data KK" class="btn bg-orange btn-flat btn-xs"><i class="fa fa-edit"></i></a>
												<?php if ($this->CI->cek_hak_akses('h')): ?>
													<a href="#" data-href="<?= site_url("keluarga/delete/$p/$o/$data[id]")?>" class="btn bg-maroon btn-flat btn-xs" title="Hapus/Keluar Dari Daftar Keluarga" data-toggle="modal" data-target="#confirm-delete"><i class="fa fa-trash-o"></i></a>
												<?php endif; ?>
											</td>
											<td class="padat">
												<div class="user-panel">
													<div class="image2">
														<img src="<?= AmbilFoto($data['foto']); ?>" class="rounded-circle" alt="Foto Penduduk"/>
													</div>
												</div>
											</td>
