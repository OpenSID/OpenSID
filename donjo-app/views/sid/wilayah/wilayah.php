							...
							<table class="table table-bordered dataTable table-striped table-hover tabel-daftar">
										...
										<td class="aksi">
											...
											<div class="btn-group">
												...
												<ul class="dropdown-menu" role="menu">
													<li>
														<a href="<?= site_url("sid_core/ajax_kantor_dusun_maps/$data[id]"); ?>h" class="btn btn-social btn-flat btn-block btn-sm"><i class='fa fa-map-marker'></i> Lokasi Kantor <?=ucwords($this->setting->sebutan_dusun)?></a>
													</li>
													<li>
														<a href="<?= site_url("sid_core/ajax_wilayah_dusun_maps/$data[id]"); ?>" class="btn btn-social btn-flat btn-block btn-sm"><i class='fa fa fa-map'></i> Peta Wilayah <?=ucwords($this->setting->sebutan_dusun)?></a>
													</li>
												</ul>
											</div>
										</td>
										<td width="25%" nowrap><?= strtoupper($data['dusun'])?></td>
										<td width="35%" nowrap><strong><?= strtoupper($data['nama_kadus'])?></strong> - <?= $data['nik_kadus']?></td>
										<td class="padat"><a href="<?= site_url("sid_core/sub_rw/$data[id]"); ?>" title="Rincian Sub Wilayah"><?= $data['jumlah_rw']?></a></td>
										<td class="padat"><?= $data['jumlah_rt']?></td>
										<td class="padat"><a href="<?= site_url("sid_core/warga_kk/$data[id]"); ?>"><?= $data['jumlah_kk']?></a></td>
										<td class="padat"><a href="<?= site_url("sid_core/warga/$data[id]"); ?>"><?= $data['jumlah_warga']?></a></td>
										<td class="padat"><a href="<?= site_url("sid_core/warga_l/$data[id]"); ?>"><?= $data['jumlah_warga_l']?></a></td>
										<td class="padat"><a href="<?= site_url("sid_core/warga_p/$data[id]"); ?>"><?= $data['jumlah_warga_p']?></a></td>
									</tr>
									<?php
								...
								<tfoot class="bg-gray disabled color-palette">
									<tr>
										...
										<th class="padat"><?= $total['total_rw'] ?: '0'; ?></th>
										<th class="padat"><?= $total['total_rt'] ?: '0'; ?></th>
										<th class="padat"><?= $total['total_kk'] ?: '0'; ?></th>
										<th class="padat"><?= $total['total_warga'] ?: '0'; ?></th>
										<th class="padat"><?= $total['total_warga_l'] ?: '0'; ?></th>
										<th class="padat"><?= $total['total_warga_p'] ?: '0'; ?></th>
									</tr>
								</tfoot>
							</table>
						</div>
