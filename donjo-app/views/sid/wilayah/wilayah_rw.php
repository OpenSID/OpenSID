			...
			<div class="col-md-12">
				<div class="box box-info">
					<div class="box-header with-border">
						...
						<a href="<?= site_url("wilayah/excel_rw/{$id_dusun}")?>" class="btn btn-social btn-flat bg-navy btn-sm visible-xs-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block" title="Unduh Data" target="_blank"><i class="fa fa-download"></i> Unduh</a>
						<a href="<?= site_url('wilayah/clear')?>" class="btn btn-social btn-flat btn-info btn-sm btn-sm visible-xs-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block" title="Kembali Ke Daftar RW">
														...
														<tbody>
															<?php foreach ($main as $data): ?>
																<tr>
																	...
																	<td nowrap>
																		<?php if (can('u')): ?>
																			<a href="<?= site_url("wilayah/urut/rw/{$paging->page}/{$data['id']}/1/{$id_dusun}"); ?>" class="btn bg-olive btn-flat btn-sm <?php if ($data['no'] == $paging->num_rows) {
																			    echo 'disabled';
																			} ?>" title="Pindah Posisi Ke Bawah"><i class="fa fa-arrow-down"></i></a>
																			<a href="<?= site_url("wilayah/urut/rw/{$paging->page}/{$data['id']}/2/{$id_dusun}"); ?>" class="btn bg-olive btn-flat btn-sm <?php if ($data['no'] == 1 && $paging->page == $paging->start_link) {
																			    echo 'disabled';
																	...
																	<?php endif; ?>
																	<td><a href="<?= site_url("wilayah/sub_rt/{$id_dusun}/{$data['id']}")?>" title="Rincian Sub Wilayah"><?= $data['jumlah_rt']?></a></td>
