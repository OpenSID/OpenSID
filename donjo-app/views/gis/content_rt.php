<div id="isi_popup_rt">
	<?php foreach ($rt_gis as $key_rt => $rt): ?>
		...
		<?php $data_title = " RT {$rt['rt']} RW {$rt['rw']} {$wilayah} {$rt['dusun']}"; ?>
			<div id="content">
				<h5 id="firstHeading" class="firstHeading">Wilayah RT <?= $rt['rt'] ?> RW <?= $rt['rw'] . ' ' . ucwords($this->setting->sebutan_dusun) . ' ' . $rt['dusun']; ?></h5>
					...
					<div id="bodyContent">
						<div class="card card-body">
							<?php foreach ($list_ref as $key => $value): ?>
								<li <?= jecho($lap, $key, 'class="active"'); ?>><a href="<?= site_url("statistik/chart_gis_rt/{$key}/{$link}") ?>" data-remote="false" data-toggle="modal" data-target="#modalBox" data-title="Statistik Penduduk <?= $data_title ?>"><?= $value; ?></a></li>
							<?php endforeach; ?>
						</div>
					</div>
				</div>
				...
				<div class="collapse box-body no-padding" id="collapseStatBantuan">
					<div class="card card-body">
						<?php foreach ($list_bantuan as $key => $value): ?>
							<li <?= jecho($lap, $key, 'class="active"'); ?>><a href="<?= site_url("statistik/chart_gis_rt/{$key}/{$link}") ?>" data-remote="false" data-toggle="modal" data-target="#modalBox" data-title="Statistik Bantuan <?= $data_title ?>"><?= $value; ?></a></li>
						<?php endforeach; ?>
					</div>
				</div>
			</div>
