			...
			<div class="col-md-12">
				<div class="card card-outline card-info">
					<div class="card-header with-border">
						<a href="<?= site_url('surat_master/form')?>" title="Tambah Format Surat" class="btn btn-flat bg-olive btn-xs visible-xs-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block visible-xl-inline-block text-left"><i class="fa fa-plus"></i> Tambah Format Surat</a>
							...
							<div class="col-sm-12">
								<div class="dataTables_wrapper dt-bootstrap no-footer">
									<form class="form-inline" id="mainform" name="mainform" action="" method="post">
										<div class="row">
											<div class="col-sm-6">
												<select class="form-control form-control-sm " name="filter" onchange="formAction('mainform','<?= site_url('surat_master/filter')?>')">
														...
														<tbody>
															<?php foreach ($main as $data): ?>
																<tr <?php if ($data['jenis']!=1): ?>style='background-color:#f8deb5 !important;'<?php endif; ?>>
																	...
																	<td><?= $data['no']?></td>
																	<td nowrap>
																		<a href="<?= site_url("surat_master/form/$p/$o/$data[id]")?>" class="btn bg-orange btn-flat btn-xs"  title="Ubah Data"><i class="fa fa-edit"></i></a>
																		<?php if ($data['kunci'] == '0'): ?>
																			<a href="<?= site_url("surat_master/lock/$data[id]/$data[kunci]")?>" class="btn bg-navy btn-flat btn-xs" title="Non-Aktifkan Surat" ><i class="fa fa-unlock"></i></a>
																			<?php if ($data['favorit']==1): ?>
																				<a href="<?= site_url("surat_master/favorit/$data[id]/$data[favorit]")?>" class="btn bg-purple btn-flat btn-xs" title="Keluarkan dari Daftar Favorit" ><i class="fa fa-star"></i></a>
																			<?php else: ?>
																				<a href="<?= site_url("surat_master/favorit/$data[id]/$data[favorit]")?>" class="btn bg-purple btn-flat btn-xs"  title="Tambahkan ke Daftar Favorit" ><i class="fa fa-star-o"></i></a>
																			<?php endif; ?>
																		<?php elseif ($data['kunci'] == '1'): ?>
																			<a href="<?= site_url("surat_master/lock/$data[id]/$data[kunci]")?>" class="btn bg-navy btn-flat btn-xs" title="Aktifkan Surat"><i class="fa fa-lock"></i></a>
																		<?php endif ?>
																		<?php if ($data['jenis']!=1): ?>
																			<a href="#" data-href="<?= site_url("surat_master/delete/$p/$o/$data[id]")?>" class="btn bg-maroon btn-flat btn-xs"  title="Hapus Data" data-toggle="modal" data-target="#confirm-delete"><i class="fa fa-trash-o"></i></a>
																		<?php endif; ?>
																	</td>
																	<td><?= $data['nama']?></td>
																	...
																	<td><?= $data['lampiran']?></td>
																	<td nowrap>
																		<a href="<?= site_url("surat_master/kode_isian/$p/$o/$data[id]")?>" class="btn btn-flat btn-info btn-xs"  title="Kode Isian"><i class="fa fa-code"></i>Kode Isian</a>
																		<a href="<?= site_url("surat_master/form_upload/$p/$o/$data[url_surat]")?>" data-remote="false" data-toggle="modal" data-target="#modalBox" title="Unggah Template Format Surat" data-title="Unggah Template Format Surat" class="btn btn-flat bg-orange btn-xs"><i class='fa fa-upload'></i> Unggah</a>
																		<?php $surat = SuratExport($data[url_surat]); ?>
																		<?php if ($surat != ""): ?>
																			<a href="<?= base_url($surat)?>" class="btn btn-flat bg-purple btn-xs"  title="Unduh Template Format Surat"><i class="fa fa-download"></i>Unduh </a>
																		<?php endif; ?>
																	</td>
