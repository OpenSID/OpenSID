			...
			<div class="ui-layout-center" id="maincontent" style="padding: 5px;">
					...
					<?php  foreach($main as $data): ?>
				      <tr <?php if($data['jenis']==1){echo "style='background-color:#ffeeaa;'";}?>>
								...
								<td>
									<div class="uibutton-group">
										<a href="<?php echo site_url("surat_master/form/$p/$o/$data[id]")?>" class="uibutton tipsy south" title="Ubah Data"><span class="fa fa-edit "> Ubah </span></a>
										<?php if($data['jenis']!=1): ?>
										...
										<?php endif;?>
										<?php if($data['kunci'] == '0'):?>
											<a href="<?php echo site_url("surat_master/lock/$data[id]/$data[kunci]")?>" class="uibutton" target="confirm" message="Non-Aktifkan Surat <?php echo $data['nama']?>?" header="Aktivasi Surat" rel="window"><span class="fa fa-unlock "></span></a>
											<a href="<?php echo site_url("surat_master/favorit/$data[id]/$data[favorit]")?>" class="uibutton" target="confirm" message="Ubah Surat <?php echo $data['nama']?> dalam daftar surat Favorit?" header="Favorit" rel="window"><span class="<?php if($data['favorit']==1){?>fa fa-star-empty  <?php }else{?> fa fa-star  <?php }?>"></span></a>
										<?php elseif($data['kunci'] == '1'): ?>
											<a href="<?php echo site_url("surat_master/lock/$data[id]/$data[kunci]")?>" class="uibutton" target="confirm" message="Aktifkan Surat <?php echo $data['nama']?>?" header="Aktivasi Surat" rel="window"><span class="fa fa-lock "></span></a>
										<?php endif?>
								...
								<td>
									<div class="uibutton-group">
										<a href="<?php echo site_url("surat_master/kode_isian/$p/$o/$data[id]")?>" class="uibutton tipsy south" title="Kode Isian"><span class="fa fa-code "> Kode Isian </span></a>
										<a href="<?php echo site_url("surat_master/form_upload/$p/$o/$data[url_surat]")?>" class="uibutton tipsy south" title="Upload Template" target="ajax-modal" rel="window" header="Upload Template"><span class="fa fa-upload-alt "> Upload </span></a>

										<?php $surat = SuratExport($data['url_surat']); ?>
										<?php if ($surat != "") { ?>
										<a href="<?php echo base_url($surat)?>" class="uibutton tipsy south" title="Unduh Template"><span class="fa fa-download-alt "> Download </span></a>
										<?php } ?>
