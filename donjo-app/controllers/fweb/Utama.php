...
class Utama extends Web_Controller
{
    ...
    public function index()
    {
        ...
        $artikel         = Artikel::withOnly(['author', 'category', 'comments'])->enable()->where('headline', '!=', Artikel::HEADLINE)->where('tgl_upload', '<=', Carbon::now())->when($cari, static fn ($q) => $q->cari($cari))->sitemap()->artikelStatis()->orderBy('tgl_upload', 'desc')->paginate(setting('web_artikel_per_page') ?? 10);
        if (! $artikel->isEmpty()) {
