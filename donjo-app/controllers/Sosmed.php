...
class Sosmed extends Admin_Controller
{
            ...
            return datatables()->of(MediaSosial::query())
                ...
                ->addColumn('aksi', static function ($row): string {
                    ...
                    if (can('u')) {
                        ...
                        if ($row->enabled == 1) {
                            $aksi .= '<a href="' . site_url("sosmed/lock/{$row->id}/1") . '" class="btn bg-navy btn-sm" title="Nonaktifkan"><i class="fa fa-unlock"></i></a> ';
                        } else {
                            $aksi .= '<a href="' . site_url("sosmed/lock/{$row->id}/0") . '" class="btn bg-navy btn-sm" title="Aktifkan"><i class="fa fa-lock"></i></a> ';
                        }
                    }
    ...
    public function lock($id = 0, $val = 0)
    {
        ...
        if ($data->update(['enabled' => $val === 0 ? 1 : 0])) {
