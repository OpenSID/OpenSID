...
class Develbar
{
    ...
    public function debug()
    {
        ...
        if ($this->default_options['enable_develbar'] == true && $this->CI->router->class != 'develbarprofiler'
            ...
            if ($this->default_options['develbar_sections']['Ajax'] == true) {
    ...
    protected function benchmarks_section($return_view = true)
    {
        ...
        foreach ($this->CI->benchmark->marker as $marker => $time) {
            if (preg_match('/(.+?)_end/i', $marker, $matches)) {
                ...
                $end   = $matches[1] . '_end';
                if (isset($this->CI->benchmark->marker[$end], $this->CI->benchmark->marker[$start])
        ...
        foreach ($this->CI->hooks->hooks as $hook_point => $_hooks) {
            ...
            foreach ($_hooks as $hook) {
                if (! array_key_exists('class', $hook)) {
                    ...
                    $total_hooks++;
                } elseif (class_exists($hook['class']) && static::class != $hook['class']) {
                    $hooks[$hook_point][] = $hook;
                    $total_hooks++;
                }
            }
    ...
    private function list_db_eloquent()
    {
        ...
        $dbName = $capsule->getDatabaseManager()->getDatabaseName();
        $host   = $capsule->getDatabaseManager()->getConfig('host') . '_ORM';
