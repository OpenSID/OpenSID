...
<script type="text/javascript">    
    function hapusLogPenduduk(elm) {
        $.post('periksaLogPenduduk/hapusLog',{ id : $(elm).data('log'), {{ $ci->security->get_csrf_token_name() }} : '{{ $ci->security->get_csrf_hash() }}' }, function(data){                
    ...
    function updateStatusPenduduk(elm) {
        $.post('periksaLogPenduduk/updateStatusDasar',{ id : $(elm).data('log'),{{ $ci->security->get_csrf_token_name() }} : '{{ $ci->security->get_csrf_hash() }}' }, function(data){                
                let _message = 'Data penduduk dengan nik {{ $nik }} gagal diperbaiki'
                let _messageClass = 'danger'
                if (data.status) {
                    let _modal = $(elm).closest('.modal')
