...
<section class="content" id="maincontent">
    ...
    <div class="box box-info">
        ...
        <div class="box-body">
            <p>{{ config_item('nama_lembaga') }} (lembaga hukum dikukuhkan Keputusan Menteri Hukum dan Hak Asasi Manusia
                ...
                pengadaan dengan {{ config_item('nama_lembaga') }} sebelum dapat menggunakan aplikasi dan layanan {{
                config_item('nama_lembaga') }} berbayar tersebut.</p>
            <p>Gunakan fitur ini untuk mendaftarkan dan mengeksekusi kerjasama resmi dengan {{
                config_item('nama_lembaga') }}. Setelah Kesepakatan Kerjasama antara Desa dan {{
                config_item('nama_lembaga') }} berlaku, Desa akan terdaftar sebagai Desa Digital {{
                config_item('nama_lembaga') }} dan berhak mengakses aplikasi dan layanan {{ config_item('nama_lembaga')
                }} berbayar dan program-program peningkatan desa digital lainnya.</p>
    ...
    <div class="box box-info">
        ...
        <form id="validasi" action="{{ site_url('pendaftaran_kerjasama/register') }}" method="POST"
            enctype="multipart/form-data" class="form-horizontal">
            <div class="box-body">
                <div class="form-group">
                    <input type="hidden" name="user_id" value="{{ $response->data->user_id ?? 0 }}">
                    <input type="hidden" name="status_langganan_id"
                        value="{{ $status = $response->data->status_langganan_id ?? 4 }}">
                    <label class="col-sm-3 control-label" for="email">Email</label>
                    <div class="col-sm-8">
                        @if ($response->data->status_langganan === 'menunggu verifikasi email')
                        <input id="email" class="form-control input-sm required" type="text"
                            placeholder="Gunakan email yang valid" name="email" value="">
                        @else
                        <input id="email" class="form-control input-sm required" type="text"
                            placeholder="Gunakan email yang valid" name="email" value="{{ $response->data->email }}">
                        @endif
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-8">
                        <input class="form-control input-sm" type="text" name="status_registrasi"
                            value="{{ $response->data->status_langganan ?? 'belum terdaftar' }}" readonly>
                    </div>
                </div>
                <div class="form-group">
                    <label class="col-sm-3 control-label" for="email">Kode {{ ucfirst(setting('sebutan_desa'))
                        }}</label>
                    <div class="col-sm-8">
                        <input class="form-control input-sm bilangan_titik required" type="text" name="desa"
                            value="{{ $response->data->desa_id ?? kode_wilayah($desa['kode_desa']) }}">
                ...
                <div class="form-group">
                    <label class="col-sm-3 control-label" for="domain">Domain {{ ucfirst(setting('sebutan_desa'))
                        }}</label>
                    <div class="col-sm-8">
                        <input id="domain" class="form-control input-sm" type="text" readonly name="domain"
                            value="{{ $response->data->domain ?? APP_URL }}">
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-8">
                        <input id="kontak_nama" class="form-control input-sm nama" type="text" name="kontak_nama"
                            value="{{ $response->data->nama_kontak }}">
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-8">
                        <input id="kontak_no_hp" class="form-control input-sm" type="number" name="kontak_no_hp"
                            value="{{ $response->data->no_hp_kontak }}">
                        ...
                        <div class="input-group input-group-sm">
                            ...
                            <span class="input-group-btn">
                                <button type="button" class="btn btn-info" id="file_browser"><i
                                        class="fa fa-search"></i> Pilih</button>
                            </span>
                            <span class="input-group-btn">
                                <a target="_blank" href="<?= site_url('pendaftaran_kerjasama/dokumen_template') ?>"
                                    type="button" class="btn btn-success"><i class="fa fa-download"></i> Unduh Dokumen
                                    Kerjasama</a>
                            </span>
                        </div>
            ...
            <div class="box-footer">
                ...
                @if (can('u'))
                <button type="button" class="simpan btn btn-social btn-info btn-sm pull-right" {{ in_array($status, [5,
                    6]) ? 'disabled' : '' }}><i class="fa fa-check"></i> Simpan</button>
                @endif
            </div>
        </form>
    </div>
