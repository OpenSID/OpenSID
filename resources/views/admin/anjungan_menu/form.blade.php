...
<div class="box box-info">
    <div class="box-header with-border">
        <a href="{{ ci_route('anjungan_menu') }}"
            class="btn btn-social btn-info btn-sm visible-xs-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block"><i
                class="fa fa-arrow-circle-left"></i> Kembali Ke Daftar Menu</a>
    </div>
    ...
    <div class="box-body">
        <div class="form-group">
            ...
            <div class="col-sm-9">
                <input name="nama" class="form-control input-sm required nomor_sk" maxlength="50" type="text"
                    value="{{ $menu['nama'] }}">
            </div>
        </div>
        <div class="form-group">
        ...
        <div class="form-group" id="jenis_link" style="{{ !$menu['link_tipe'] && (print 'display:none;') }}">
            ...
            <div class="col-sm-9">
                <select id="artikel_statis" class="form-control input-sm jenis_link select2"
                    name="{{ jecho($menu['link_tipe'], 1, 'link') }}"
                    style="{{ $menu['link_tipe'] != 1 && (print 'display:none;') }}">
                    <option value="">-- Pilih Artikel Statis --</option>
                    @foreach ($artikel_statis as $data)
                    <option value="artikel/{{ $data['id'] }}" {{ selected($menu['link'], "artikel/{$data['id']}" ) }}>{{
                        $data['judul'] }}</option>
                    @endforeach
                </select>
                <select id="kategori_artikel" class="form-control input-sm jenis_link"
                    name="{{ jecho($menu['link_tipe'], 8, 'link') }}"
                    style="{{ $menu['link_tipe'] != 8 && (print 'display:none;') }}">
                    <option value="">-- Pilih Kategori Artikel --</option>
                    @foreach ($kategori_artikel as $data)
                    <option value="kategori/{{ $data['slug'] }}" {{ selected($menu['link'], "kategori/{$data['slug']}" )
                        }}>{{ $data['kategori'] }}</option>
                    @endforeach
                </select>
                <select id="statistik_penduduk" class="form-control input-sm jenis_link"
                    name="{{ jecho($menu['link_tipe'], 2, 'link') }}"
                    style="{{ $menu['link_tipe'] != 2 && (print 'display:none;') }}">
                    <option value="">-- Pilih Statistik Penduduk --</option>
                    @foreach ($statistik_penduduk as $id => $nama)
                    <option value="{{ " statistik/{$id}" }}" {{ selected($menu['link'], "statistik/{$id}" ) }}>{{ $nama
                        }}</option>
                    @endforeach
                </select>
                <select id="statistik_keluarga" class="form-control jenis_link input-sm"
                    name="{{ jecho($menu['link_tipe'], 3, 'link') }}"
                    style="{{ $menu['link_tipe'] != 3 && (print 'display:none;') }}">
                ...
                <select id="statistik_program_bantuan" class="form-control input-sm jenis_link"
                    name="{{ jecho($menu['link_tipe'], 4, 'link') }}"
                    style="{{ $menu['link_tipe'] != 4 && (print 'display:none;') }}">
                    ...
                    <option value="{{ " statistik/{$nama['lap']}" }}" {{
                        selected($menu['link'], "statistik/{$nama['lap']}" ) }}>{{ $nama['nama'] }}</option>
                    @endforeach
                </select>
                <select id="statis_lainnya" class="form-control input-sm jenis_link"
                    name="{{ jecho($menu['link_tipe'], 5, 'link') }}"
                    style="{{ $menu['link_tipe'] != 5 && (print 'display:none;') }}">
                    <option value="">-- Pilih Halaman Statis Lainnya --</option>
                    @foreach ($statis_lainnya as $id => $nama)
                    <option value="{{ $id }}" {{ selected($menu['link'], $id) }}>{{ str_replace('[Desa]',
                        ucwords(setting('sebutan_desa')), $nama) }}</option>
                    @endforeach
                </select>
                <select id="artikel_keuangan" class="form-control input-sm jenis_link"
                    name="{{ jecho($menu['link_tipe'], 6, 'link') }}"
                    style="{{ $menu['link_tipe'] != 6 && (print 'display:none;') }}">
                    <option value="">-- Pilih Artikel Keuangan --</option>
                    @foreach ($artikel_keuangan as $data)
                    <option value="{{ $data['id'] }}" {{ selected($menu['link'], $data['id']) }}>{{ $data['judul'] }}
                    </option>
                    @endforeach
                </select>
                <select id="kelompok" class="form-control input-sm jenis_link required"
                    name="{{ jecho($menu['link_tipe'], 7, 'link') }}"
                    style="{{ $menu['link_tipe'] != 7 && (print 'display:none;') }}">
                    <option value="">Pilih Kelompok</option>
                    @foreach ($kelompok as $kel)
                    <option value="{{ " data-kelompok/{$kel['id']}" }}" {{
                        selected($menu['link'], "data-kelompok/{$kel['id']}" ) }}>{{ $kel['nama'] . ' (' .
                        $kel['master'] . ')' }}</option>
                    @endforeach
                </select>
                <select id="lembaga" class="form-control input-sm jenis_link required"
                    name="{{ jecho($menu['link_tipe'], 7, 'link') }}"
                    style="{{ $menu['link_tipe'] != 7 && (print 'display:none;') }}">
                    <option value="">Pilih Lembaga</option>
                    @foreach ($lembaga as $lem)
                    <option value="{{ " data-lembaga/{$lem['id']}" }}" {{
                        selected($menu['link'], "data-lembaga/{$lem['id']}" ) }}>{{ $lem['nama'] . ' (' . $lem['master']
                        . ')' }}</option>
                    @endforeach
                </select>
                <select id="suplemen" class="form-control input-sm jenis_link required"
                    name="{{ jecho($menu['link_tipe'], 9, 'link') }}"
                    style="{{ $menu['link_tipe'] != 9 && (print 'display:none;') }}">
                    <option value="">Pilih Suplemen</option>
                    @foreach ($suplemen as $sup)
                    <option value="{{ " data-suplemen/{$sup['id']}" }}" {{
                        selected($menu['link'], "data-suplemen/{$sup['id']}" ) }}>{{ $sup['nama'] }}</option>
                    @endforeach
                </select>
                <select id="status_idm" class="form-control input-sm jenis_link required"
                    name="{{ jecho($menu['link_tipe'], 10, 'link') }}"
                    style="{{ $menu['link_tipe'] != 10 && (print 'display:none;') }}">
                    <option value="">Pilih Tahun</option>
                    @foreach (tahun(2020) as $thn)
                    <option value="{{ " status-idm/{$thn}" }}" {{ selected($menu['link'], "status-idm/{$thn}" ) }}>{{
                        $thn }}</option>
                    @endforeach
                </select>
            </div>
    ...
    <div class="form-group" id="jenis_link" style="{{ !$menu['link_tipe'] && (print 'display:none;') }}">
        ...
        <div class="col-sm-9">
            ...
            <select id="statistik_program_bantuan" class="form-control input-sm jenis_link"
                ...
                @endforeach
                @foreach ($statistik_program_bantuan as $nama)
                <option value="{{ " statistik/{$nama['lap']}" }}" {{ selected($menu['link'], "statistik/{$nama['lap']}"
                    ) }}>{{ $nama['nama'] }}</option>
                @endforeach
            </select>
            <select id="statis_lainnya" class="form-control input-sm jenis_link"
