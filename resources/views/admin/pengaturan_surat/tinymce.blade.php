...
<div class="tab-pane" id="form-isian">
    ...
    <div class="nav-tabs-custom">
        <ul class="nav nav-tabs customized-tab" id="tabs">
            ...
            <li id="list-{{$item}}" data-name="{{$item}}"><a href="#tab-{{ $item }}" data-toggle="tab">{{
                    str_replace('_', ' ', $item) }}</a></li>
            @empty
        ...
        <div class="tab-content custom">
            <div class="tab-pane active" id="form-utama">
                <div class="box-body">
                    <button type="button" class="utama-delete btn btn-danger btn-sm pull-right hide" onclick="deleteTab(event)"><i
            ...
            <div class="tab-pane" id="tab-{{$item}}">
                ...
                <div class="box-body">
                    <button type="button" class="btn btn-danger btn-sm pull-right" data-kategori="{{$item}}" onclick="deleteTab(event)"><i
                            class="fa  fa-times-circle"></i> Hapus Tab</button>
                                ...
                                <tr class="warga_desa">
                                    ...
                                    <td>
                                        <select class="form-control input-sm kategori select2"
                                            ...
                                            <option value="">SEMUA</option>
                                            @foreach ($form_isian['daftar_jenis_kelamin'] as $key => $data)
                                            <option value="{{ $key }}" @selected($key==$suratMaster->
                                                form_isian->$item->sex)>{{ $data }}
                                            </option>
                                            @endforeach
                                        </select>
                                    </td>
                                ...
                                <tr class="warga_desa">
                                    ...
                                    <td>
                                        <select class="form-control input-sm select2 kategori"
                                            ...
                                            @foreach ($form_isian['daftar_status_dasar'] as $key => $data)
                                            <option value="{{ $key }}" @selected($key==$suratMaster->
                                                form_isian->$item->status_dasar)>{{ $data }}
                                            </option>
                                            @endforeach
                                        </select>
                                    </td>
...
<div class="modal fade" id="modal-tab" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        ...
        <div class="modal-content">
            ...
            <div id="form-new-tab">
                ...
                <div class="modal-footer">
                    <button type="submit" class="btn btn-social btn-info btn-sm" id="btn-new-tab" data-backdrop="false"
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('#btn-new-tab').click(function(e) {
                ...
                if (checkInput == 'huruf') {
                    $('#error_category').show()
                    $('#error_category').text('Maksimal 10 huruf.')
                    return false;
                }
                if (checkInput == 'angka') {
                    $('#error_category').show()
                ...
                $("#form-utama").clone(true)
                    .map(function() {
                        ...
                        selectElements.forEach((elselect2) => {
                            ...
                            if(elselect2.classList.contains('select2')) {
                                ...
                                if(elselect2.nextElementSibling != null) elselect2.nextElementSibling.remove();
                    ...
                    if (elkodeIsian != null) {
                        ...
                        trkode.forEach((tr, index) => {
                            var kategori_isian = tr.querySelectorAll('.kode-isian input, .kode-isian textarea, .kode-isian select, .kode-isian');
                            ...
                            kategori_isian.forEach((elselect2) => {
                                    ...
                                    if(oldname != null) {
                                        ...
                                        var fullname = `kategori_${fixname}[${nama_kategori}][]`
                                        console.log(elselect2.classList)
                                        if(oldname == 'pilihan_kode[0][]') {
                                            fullname = `kategori_${fixname}[${nama_kategori}][0][]`
                                            if(elselect2.classList.contains('select2')) {
                                                    console.log(123);
                ...
                var newNavItem = $('<li><a href="#' + newTabId + '" data-toggle="tab">' + nama_kategori
                    .replace(/_/g, ' ') + '</a></li>');
        ...
        function validateInputCategory(input) {
            ...
            var isWithinMaxLength = trimmedInput.length <= 10;
