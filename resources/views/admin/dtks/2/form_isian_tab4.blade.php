<div class="row">
    ...
    <div class="col-sm-12 text-center">
        <button type="button" onclick="$(`#nav-bagian-3`).trigger('click')"
            class="btn btn-social btn-default btn-sm"><i class='fa fa-arrow-left'></i> Sebelumnya</button>
        <button type="button" onclick="$(`#nav-bagian-5`).trigger('click')"
            class="btn btn-social btn-default btn-sm">Selanjutnya <i class="fa fa-arrow-right"></i></button>
    </div>
</div>
...
<script>
    ...
    var global_table_art_dtsk = [
        {id: 'tabel_ket_demografi', title_select:'Keterangan Demografi'},
        {id: 'tabel_pendidikan', title_select:'Pendidikan (Untuk Anggota Keluarga 5 Tahun ke Atas)'},
        {id: 'tabel_ketenagakerjaan', title_select:'Ketenagakerjaan (Untuk Anggota Keluarga 5 Tahun ke Atas)'},
        {id: 'tabel_kepemilikan_usaha', title_select:'Kepemilikan Usaha (Untuk Anggota Keluarga 5 Tahun ke Atas)'},
        {id: 'tabel_kesehatan', title_select:'Kesehatan'},
        {id: 'tabel_program_perlindungan_sosial', title_select:'Program Perlindungan Sosial'},
    ];;
    ...
    function refresh_jumlah_dan_terisi(id_anggota){
        $('#tabel_art_dtks tr[data-id="' + id_anggota + '"] a.modal-table').each(function(index, el){
            let indexTR = global_table_art_dtsk.findIndex(function(item){ return item.id == $(el).data('table');});
            if(indexTR != -1){
    ...
    function setJumlahTerisi(index_agt = null){
        let pengaturan = function(element, index_anggota) {
            ...
            let temp_jumlah_terisi = [
                {jumlah: 10, terisi:0}, // id: 'tabel_ket_demografi'
                {jumlah: 4, terisi:0}, // id: 'tabel_pendidikan'
                {jumlah: 6, terisi:0}, // id: 'tabel_ketenagakerjaan'
                {jumlah: 9, terisi:0}, // id: 'tabel_kepemilikan_usaha'
                {jumlah: 28, terisi:0}, // id: 'tabel_kesehatan'
                {jumlah: 6, terisi:0}, // id: 'tabel_program_perlindungan_sosial'
            ];
            ...
            // -- ket demografi
            indexTR = 0; terisi = 0; temp_jumlah_terisi[indexTR].jumlah = 8; // default value
            ...
            // -- pendidikan
            indexTR = 1; terisi = 0; temp_jumlah_terisi[indexTR].jumlah = 4; // default value
            if(tmp_umur >= 5){
            ...
            // -- ketenagakerjaan
            indexTR = 2; terisi = 0; temp_jumlah_terisi[indexTR].jumlah = 6; // default value
            if(tmp_umur >= 5){
            ...
            // -- kepemilikan usaha
            indexTR = 3; terisi = 0; temp_jumlah_terisi[indexTR].jumlah = 9; // default value
            if(tmp_umur >= 5){
                if(['1'].indexOf(global_data_anggota[index_anggota].kd_punya_usaha_sendiri_bersama) > -1){
                    ['kd_punya_usaha_sendiri_bersama', 'jumlah_usaha_sendiri_bersama', 'kd_lapangan_usaha_dr_usaha', 'tulis_lapangan_usaha_dr_usaha', 'jumlah_pekerja_dibayar', 'jumlah_pekerja_tidak_dibayar', 'kd_kepemilikan_ijin_usaha', 'kd_omset_usaha_perbulan', 'kd_guna_internet_usaha'].forEach(function_cek_sudah_diisi);
                }else{
                    temp_jumlah_terisi[indexTR].jumlah = 1;
            ...
            // -- kesehatan
            indexTR = 4; terisi = 0; temp_jumlah_terisi[indexTR].jumlah = 28; // default value
            ...
            if(tmp_umur >= 60
                || ['1', '2'].indexOf(global_data_anggota[index_anggota].kd_sering_sedih_depresi)
                || ['1', '2'].indexOf(global_data_anggota[index_anggota].kd_sulit_penglihatan)
                || ['1', '2'].indexOf(global_data_anggota[index_anggota].kd_sulit_pendengaran)
                || ['1', '2'].indexOf(global_data_anggota[index_anggota].kd_sulit_jalan_naiktangga)
                || ['1', '2'].indexOf(global_data_anggota[index_anggota].kd_sulit_gerak_tangan_jari)
                || ['1', '2'].indexOf(global_data_anggota[index_anggota].kd_sulit_belajar_intelektual)
                || ['1', '2'].indexOf(global_data_anggota[index_anggota].kd_sulit_perilaku_emosi)
                || ['1', '2'].indexOf(global_data_anggota[index_anggota].kd_sulit_paham_bicara_kom)
                || ['1', '2'].indexOf(global_data_anggota[index_anggota].kd_sulit_mandiri)
                || ['1', '2'].indexOf(global_data_anggota[index_anggota].kd_sulit_ingat_konsentrasi)
                || ['1', '2'].indexOf(global_data_anggota[index_anggota].kd_sering_sedih_depresi)
            ){
            ...
            indexTR = 5; terisi = 0; temp_jumlah_terisi[indexTR].jumlah = 6; // default value
    ...
    $(document).ready(function() {
        ...
        $('.modal-table').on('click', function(ev){
            ...
            let index_anggota = global_data_anggota.findIndex(function(item){ return item.id == id_anggota;});
            // set global jumlah dan terisi
            global_art_jumlah_terisi = global_data_anggota[index_anggota].jumlah_terisi;
            // refresh select title
            $('#tab4_kelompok_pertanyaan option').each(function(index, el){
                let indexTR = global_table_art_dtsk.findIndex(function(item){ return item.id == $(el).val();});
                if(indexTR != -1){
            ...
            $(table).find('tr').each(function(){ $(this).show(); });
            ...
            if(table == 'tabel_ket_demografi'){
                ...
                // tampilkan semua
                    ['tr_4_412', 'tr_4_413', 'tr_4_414', 'tr_4_415'].forEach(function(el){ $('#' + el).show(); });
                // cek
                if(global_umur_art < 5){
                    ['tr_4_412', 'tr_4_413', 'tr_4_414', 'tr_4_415'].forEach(function(el){ $('#' + el).hide(); });
                }else{
                ...
                // tampilkan semua
                ['tr_4_416a', 'tr_4_416b', 'tr_4_417', 'tr_4_lapangan_usaha_pekerjaan', 'tr_4_418', 'tr_4_419'].forEach(function(el){ $('#' + el).show(); });
                // cek
                if(global_umur_art < 5){
                    ['tr_4_416a', 'tr_4_416b', 'tr_4_417', 'tr_4_lapangan_usaha_pekerjaan', 'tr_4_418', 'tr_4_419'].forEach(function(el){ $('#' + el).hide(); });
                }else{
                ...
                // tampilkan semua
                    ['tr_4_420a', 'tr_4_420b', 'tr_4_421', 'tr_4_lapangan_usaha_dr_usaha', 'tr_4_422', 'tr_4_423', 'tr_4_424', 'tr_4_425', 'tr_4_426'].forEach(function(el){ $('#' + el).show(); });
                // cek
                if(global_umur_art < 5){
                    ['tr_4_420a', 'tr_4_420b', 'tr_4_421', 'tr_4_lapangan_usaha_dr_usaha', 'tr_4_422', 'tr_4_423', 'tr_4_424', 'tr_4_425', 'tr_4_426'].forEach(function(el){ $('#' + el).hide(); });
                }else{
