        ...
        <div class="box box-info">
            <form action="{{ $form_action }}" method="POST" enctype="multipart/form-data" class="form-horizontal">
                ...
                <div class='box-footer'>
                    ...
                    @if (can('u'))
                        <a href="#" data-href="{{ route("garis.kosongkan", implode('/',[$parent, $garis['id']])) }}" class="btn btn-social bg-maroon btn-sm visible-xs-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block" title="Kosongkan Wilayah" data-toggle="modal" data-target="#confirm-status" data-body="Apakah yakin akan mengosongkan peta wilayah ini?"><i class="fa fa fa-trash-o"></i>Kosongkan</a>
                    @endif
...
<script>
    window.onload = function() {        
        let jenis = "{{ $garis['line']['jenis'] }}"
        let tebal = "{{ $garis['line']['tebal'] }}"
        let warna = "{{ $garis['line']['color'] }}"
        ...
        var peta_garis = L.map('tampil-map', pengaturan_peta).setView(posisi, zoom);
