<div class="box box-info">
    <form id="validasi" action="{{ ci_route('modul.ubah_server') }}" method="POST" class="form-horizontal">
        <div class="box-body">
            ...
            <div class="form-group">
                <label class="col-sm-3 control-label">Penggunaan {{ config_item('nama_aplikasi') }} di {{
                    ucwords(setting('sebutan_desa')) }}</label>
                <div class="col-sm-9 col-lg-4">
                    <select class="form-control required input-sm" name="jenis_server"
                        onchange="ubah_jenis_server($(this).val())">
                        <option value='' selected="selected">-- Pilih Penggunaan {{ config_item('nama_aplikasi') }} --
                        </option>
            ...
            <div class="form-group" id="offline_online_hosting"
                style="@if (!in_array(setting('penggunaan_server'), ['3', '5', '6'])) display: none; @endif ">
            ...
            <div class="form-group" id="offline_ada_hosting"
                style="@if (setting('penggunaan_server') != '5') display: none; @endif">
            ...
            <div class="form-group" id="offline_saja"
                style="@if (setting('penggunaan_server') != '1') display: none; @endif ">
                <label class="col-sm-3 control-label">Akses web pada server offline ini</label>
                <div class="col-sm-9 col-lg-4">
                    <select class="form-control input-sm" name="offline_mode_saja">
                        <option value='' selected="selected">-- Pilih Akses Web --</option>
                        @foreach (\App\Enums\OfflineModeEnum::all() as $key => $item)
                        <option value="{{ $key }}" @selected(setting('penggunaan_server')=='1' &&
                            setting('offline_mode')==$key)>{{ $item }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
    ...
    <div class="box-body">
        <div class="alert alert-info">
            <p>Server ini hanya digunakan untuk menampilkan data bagi publik. Secara default, semua modul dinon-aktifkan
                kecuali menu Pengaturan dan Admin Web. Pengelolaan data penduduk dan lain-lain dilakukan di server
                terpisah, secara offline di Kantor Desa. Untuk memutakhirkan data di
        ...
        <a href="#" data-title="Acak Data" class="btn btn-social btn-danger btn-sm" data-toggle="modal"
            data-target="#confirm-acak"><i class='fa fa-trash-o'></i>Acak Data</a>
        <a href="{{ ci_route('database.mutakhirkan_data_server') }}" title="Sinkronkan Data" data-remote="false"
            data-toggle="modal" data-target="#modalBox" data-title="Sinkronkan Data"
            class="btn btn-social btn-info btn-sm"><i class="fa fa-refresh"></i>Impor Data Mutakhir</a>
    </div>
    @endif
</div>
@push('scripts')
