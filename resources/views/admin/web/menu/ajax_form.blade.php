    ...
    <div class="modal-body">
        ...
        <div class="form-group">
            ...
            <select class="form-control input-sm required" id="link_tipe" name="link_tipe"
                onchange="ganti_jenis_link(this);">
        ...
        <div class="form-group" id="jenis_link" style="@if (!$menu['link_tipe']) display:none; @endif">
            ...
            <select id="artikel_statis" class="form-control input-sm jenis_link select2"
                ...
                style="@if ($menu['link_tipe'] != 1) display:none; @endif">
            ...
            <select id="kategori_artikel" class="form-control input-sm jenis_link"
                ...
                style="@if ($menu['link_tipe'] != 8) display:none; @endif">
            ...
            <select id="statistik_penduduk" class="form-control input-sm jenis_link"
                ...
                style="@if ($menu['link_tipe'] != 2) display:none; @endif">
            ...
            <select id="statistik_keluarga" class="form-control jenis_link input-sm"
                ...
                style="@if ($menu['link_tipe'] != 3) display:none; @endif">
            ...
            <select id="statistik_program_bantuan" class="form-control input-sm jenis_link"
                ...
                style="@if ($menu['link_tipe'] != 4) display:none; @endif">
                ...
                <option value="statistik/50{{ $nama['id'] }}" @selected($menu['link']=="statistik/50{$nama['id']}" )>{{
                    $nama['nama'] }}</option>
                @endforeach
            </select>
            <select id="statis_lainnya" class="form-control input-sm jenis_link"
                name="{{ jecho($menu['link_tipe'], 5, 'link') }}"
                style="@if ($menu['link_tipe'] != 5) display:none; @endif">
                <option value="">-- Pilih Halaman Statis Lainnya --</option>
                @foreach ($statis_lainnya as $id => $nama)
                <option value="{{ $id }}" @selected($menu['link']==$id)>{{ str_replace(['[Pemerintah Desa]', '[Desa]'],
                    [ucwords(setting('sebutan_pemerintah_desa')), ucwords(setting('sebutan_desa'))], $nama) }}</option>
                @endforeach
            </select>
            <select id="artikel_keuangan" class="form-control input-sm jenis_link"
                name="{{ jecho($menu['link_tipe'], 6, 'link') }}"
                style="@if ($menu['link_tipe'] != 6) display:none; @endif">
            ...
            <select id="kelompok" class="form-control input-sm jenis_link required"
                ...
                style="@if ($menu['link_tipe'] != 7) display:none; @endif">
                <option value="">Pilih Kelompok</option>
                @foreach ($kelompok as $kel)
                <option value="data-kelompok/{{ $kel['id'] }}" @selected($menu['link']=="data-kelompok/{$kel['id']}" )>
                    {{ $kel['nama'] . ' (' . $kel['master'] . ')' }}</option>
                @endforeach
            </select>
            <select id="lembaga" class="form-control input-sm jenis_link required"
                ...
                <option value="data-lembaga/{{ $lem['id'] }}" @selected($menu['link']=="data-lembaga/{$lem['id']}" )>{{
                    $lem['nama'] . ' (' . $lem['master'] . ')' }}</option>
                @endforeach
            </select>
            <select id="suplemen" class="form-control input-sm jenis_link required"
                name="{{ jecho($menu['link_tipe'], 9, 'link') }}"
                style="@if ($menu['link_tipe'] != 9) display:none; @endif">
                <option value="">Pilih Suplemen</option>
                @foreach ($suplemen as $sup)
                <option value="data-suplemen/{{ $sup['id'] }}" @selected($menu['link']=="data-suplemen/{$sup['id']}" )>
                    {{ $sup['nama'] }}</option>
                @endforeach
            </select>
            <select id="status_idm" class="form-control input-sm jenis_link required"
                name="{{ jecho($menu['link_tipe'], 10, 'link') }}"
                style="@if ($menu['link_tipe'] != 10) display:none; @endif">
            ...
            <span id="eksternal" class="jenis_link" style="@if ($menu['link_tipe'] != 99) display:none; @endif">
                <input name="{{ jecho($menu['link_tipe'], 99, 'link') }}" class="form-control input-sm" type="text"
                    value="{{ $menu['link'] }}" />
