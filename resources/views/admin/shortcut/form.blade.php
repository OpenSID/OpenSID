...
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('shortcut') }}">Shortcut</a></li>
                ...
                {!! form_open($form_action, 'id="validasi"') !!}
                    <div class="box-body">
                        ...
                        <div class="row">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2" id="akses" name="akses" data-placeholder="Pilih Akses Modul">
                                        ...
                                        @foreach ($moduls as $kslug => $mdl)
                                            <option value="{{ $kslug }}" @selected($kslug === $shortcut->akses)>{{ SebutanDesa($mdl) }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-8">
                                <div class="form-group">
                                    ...
                                    <code id="url_hasil">Hasil : {{ site_url() }}</code>
                                </div>
                            </div>
                        </div>
                        <div class="row">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2-ikon required" id="icon" name="icon" data-placeholder="Pilih Icon">
                                        @foreach ($icons as $icon)
                                            <option value="{{ $icon }}" @selected($icon === $shortcut->icon)>{{ $icon }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2" name="status">
                                        @foreach (\App\Enums\StatusEnum::all() as $key => $value)
                                            <option value="{{ $key }}" @selected($key == $shortcut->status)>{{ $value }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2" name="query_otomatis" data-placeholder="Pilih Query">
                                        ...
                                        @foreach ($querys as $query)
                                            <option value="{{ $query }}" @selected($query === $shortcut->raw_query)>Jumlah {{ $query }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="form-group" id="query_manual" {!! in_array($shortcut->jenis_query, [null, 0]) ? 'style="display: none;"' : '' !!}>
                            ...
                            <textarea name="query_manual" class="form-control input-sm" rows="5" placeholder="Contoh : SELECT COUNT(*) as Jumlah FROM surat WHERE status='1'">{{ $shortcut->query }}</textarea>
                        </div>
                        <div class="row">
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label>Urutan</label>
                                    <input name="urutan" class="form-control input-sm" type="number" min="1" value="{{ $shortcut->urutan }}">
                                </div>
                            </div>
                        </div>
                    </div>
