...
<div class="box box-solid">
    ...
    <div class="box-header with-border">
        <form method="get" class="form-inline text-center">
            ...
            <input type="text" name="keyword" maxlength="50" class="form-control" value="{{ $keyword }}"
    ...
    <div class="box-body">
                ...
                <div class="card mb-4 box-shadow">
                    ...
                    <div id="carousel-produk{{ $in }}" class="carousel slide" data-ride="carousel">
                        <ol class="carousel-indicators">
                            @for ($i = 0; $i < setting('banyak_foto_tiap_produk'); $i++) @if (!empty($foto[$i])) <li
                                data-target="#carousel-produk{{ $in }}" data-slide-to="{{ $i }}"
                                class="{{ $i == 0 ? 'active' : '' }}">
                                </li>
                                @endif
                                @endfor
                        ...
                        <div class="carousel-inner">
                            @for ($i = 0; $i < setting('banyak_foto_tiap_produk'); $i++) @if (!empty($foto[$i])) <div
                                class="item {{ $i == 0 ? 'active' : '' }}">
                                @if (is_file(LOKASI_PRODUK . $foto[$i]))
                                <img class="image-produk card-img-top" src="{{ base_url(LOKASI_PRODUK . $foto[$i]) }}"
                                    alt="Produk {{ $i + 1 }}">
                                @else
                                <img class="card-img-top" style="width: auto; max-height: 170px;"
                                    src="{{ asset('images/404-image-not-found.jpg') }}" alt="Foto Produk">
                                @endif
                        </div>
                        @endif
                        @endfor
                    ...
                    <div class="d-flex justify-content-between align-items-center" style="margin-bottom: 30px;">
                        <div class="btn-group">
                            ...
                            <a class="btn btn-sm btn-success"
                                href="https://api.whatsapp.com/send?phone={{ format_telpon($pro->telepon) }}&amp;text={{ $pesan }}"
                                rel="noopener noreferrer" target="_blank" title="WhatsApp">
                                <i class="fa fa-whatsapp"></i> Beli
                            </a>
                            @endif
                            <a class="btn btn-sm btn-warning lokasi-pelapak" data-remote="false" data-toggle="modal"
                                data-target="#map-modal" title="Lokasi" data-lat="{{ $pro->lat }}"
                                data-lng="{{ $pro->lng }}" data-zoom="{{ $pro->zoom }}"
                                data-title="Lokasi Pelapak ({{ $pro->pelapak }})"><i class="fa fa fa-map"></i>
                                Lokasi</a>
                            <a class="btn btn-sm btn-primary text-white" data-remote="false" data-toggle="modal"
                                data-target="#descModal{{ $in }}" title="Deskripsi"><i class="fa fa-info-circle"></i>
                                Deskripsi</a>
                        </div>
                    </div>
                </div>
        ...
        <!-- Modal Deskripsi -->
        <div class="modal fade" id="descModal{{ $in }}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
            aria-hidden="true">
