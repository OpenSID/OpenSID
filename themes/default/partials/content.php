					...
					<div class=\"teks\">
						<div class=\"kecil\">
							...
							if (trim($data['kategori']) != '') {
								echo " <i class='fa fa-tag'></i> <a href='", site_url("first/kategori/$data[id_kategori]") ,"'>$data[kategori]</a>";
							}
						echo "
						</div>
						<div class=\"img\">";
...
if($artikel){
	...
	<div>Halaman ".$p." dari ".$paging->end_link."</div>
		...
		if($paging->next){
			echo "<li><a href=\"".site_url("first/".$paging_page."/$paging->next" . $paging->suffix)."\" title=\"Halaman Selanjutnya\"><i class=\"fa fa-forward\"></i>&nbsp;</a></li>";
		}
		if($paging->end_link){
			echo "<li><a href=\"".site_url("first/".$paging_page."/$paging->end_link" . $paging->suffix)."\" title=\"Halaman Terakhir\"><i class=\"fa fa-fast-forward\"></i>&nbsp;</a></li>";
		}
			echo "";
		echo "
		</ul>
