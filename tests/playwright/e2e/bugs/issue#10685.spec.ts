...
test.describe('Bug/error: Tampilan Previews Cetak Pada Halaman Buku Administrasi Umum - Buku Tanah di Desa Terpotong Setengah dan Nama Penandatangan tidak muncul #10685', () => {
  test('fix: perbaiki dan refaktor preview cetak/unduh pada buku tanah di desa', {
    annotation: {
      type: 'issue',
      description: 'https://github.com/OpenSID/OpenSID/issues/#10685',
    },
  }, async ({ page }) => {
    // 1. Akses halaman Buku Tanah di Desa
    await page.goto('bumindes_tanah_desa');

    // 2. Pastikan halaman sudah loading dengan benar
    await expect(page.locator('text=Buku Tanah di').first()).toBeVisible();
    ...
    await expect(frameContent.locator('text=NAMA PERORANGAN')).toBeVisible();
    await expect(frameContent.locator('text=STATUS HAK TANAH')).toBeVisible();
