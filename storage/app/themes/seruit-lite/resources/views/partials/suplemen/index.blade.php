...
>
    <nav role="navigation" aria-label="navigation" class="breadcrumb text-sm mb-4">
        <ol class="flex items-center justify-center space-x-2 text-gray-500 dark:text-gray-400">
            ...
            <li aria-current="page" class="font-medium text-gray-700 dark:text-gray-300">Data Suplemen</li>
        </ol>
    </nav>
    <div class="flex items-center mt-6">
        <div class="flex-grow border-t border-gray-300 dark:border-gray-700"></div>
        <h1 class="flex-shrink px-4 text-2xl lg:text-3xl font-bold uppercase text-center" x-text="judulHalaman"></h1>
        <div class="flex-grow border-t border-gray-300 dark:border-gray-700"></div>
    </div>
    ...
    <div x-show="isLoading" class="text-center py-12">
        <div class="flex items-center justify-center space-x-2 text-gray-500">
            ...
            <span class="font-semibold">Memuat Data Suplemen...</span>
        </div>
    </div>

    <div x-show="!isLoading" x-cloak class="mt-8 space-y-8">
        <div>
            <h3 class="text-xl font-bold mb-4">Rincian Data</h3>
            <div class="p-4 bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-700 grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                <div>
                    <strong class="block text-gray-500 dark:text-gray-400">Nama Data</strong>
                    <span x-text="detail.nama || '-'"></span>
                </div>
                <div>
                    <strong class="block text-gray-500 dark:text-gray-400">Sasaran Terdata</strong>
                    <span x-text="detail.nama_sasaran || '-'"></span>
                </div>
                <div>
                    <strong class="block text-gray-500 dark:text-gray-400">Keterangan</strong>
        ...
        <div>
            <h3 class="text-xl font-bold mb-4">Daftar Terdata</h3>
            <div class="overflow-x-auto">
                <table class="w-full text-sm border-collapse" id="tabel-terdata">
...
@push('scripts')
<script>
    function suplemenData() {
        return {
            isLoading: true,
            judulHalaman: 'Data Suplemen',
            detail: {},
            dataTable: null,
            
            init() {
                this.loadDetailSuplemen();
            },
            ...
            initDataTable(suplemenId) {
                ...
                this.dataTable = $('#tabel-terdata').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: {
