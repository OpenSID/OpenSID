...
$(document).ready(function(){if($('#peserta_program').length){let pesertaDatatable=$('#peserta_program').DataTable({processing:!0,serverSide:!0,order:[],ajax:{url:bantuanUrl,type:'POST',data:function(row){return{"page[size]":row.length,"page[number]":(row.start/row.length)+1,"filter[search]":row.search.value,"sort":(row.order[0]?.dir==="asc"?"":"-")+row.columns[row.order[0]?.column]?.name,}},dataSrc:function(json){json.recordsTotal=json.meta.pagination.total
