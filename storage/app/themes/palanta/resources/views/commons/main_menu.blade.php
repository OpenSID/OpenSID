...
<nav class="bg-primary-100 text-white hidden lg:block" role="navigation">
  <ul>
    ...
    <li class="inline-block relative" @if($has_dropdown) x-data="{dropdown: false}" @endif>
      ...
      <a href="{{ $menu_link }}" class="p-3 inline-block hover:bg-primary-200" @mouseover="dropdown = true"
        @mouseleave="dropdown = false" @click="dropdown = !dropdown" @if($has_dropdown) aria-expanded="false"
        aria-haspopup="true" @endif>
        {{ $menu['nama'] }}

        @if($has_dropdown)
        <i class="fas fa-chevron-down text-xs ml-1 inline-block transition duration-300"
          :class="{'transform rotate-180': dropdown}"></i>
        @endif
      </a>
      ...
      <ul
        ...
        <li class="inline-block relative"><a href="{{ $childrens['link_url'] }}"
            class="block py-3 pl-5 pr-4 hover:bg-primary-200 hover:text-white">{{ $childrens['nama'] }}
            @if($has_dropdown)
            <i class="fas fa-chevron-left text-xs ml-1 inline-block transition duration-300"
              :class="{'transform rotate-180': dropdown}"></i>
            @endif
          </a></li>
        ...
        <li class="inline-block relative" @if($bhas_dropdown) x-data="{dropdown: false}" @endif>
          ...
          <a href="{{ $bmenu_link }}" class="p-3 inline-block hover:bg-primary-200" @mouseover="dropdown = true"
            @mouseleave="dropdown = false" @click="dropdown = !dropdown" @if($bhas_dropdown) aria-expanded="false"
            aria-haspopup="true" @endif>
            {{ $bmenu['nama'] }}
          ...
          <ul
            ...
            <li><a href="{{ $bchildrens['link_url'] }}"
                class="block py-3 pl-5 pr-4 hover:bg-primary-200 hover:text-white">{{ $bchildrens['nama'] }}</a></li>
            @endforeach
        ...
        @endforeach
        @else
        <li><a href="{{ $childrens['link_url'] }}" class="block py-3 pl-5 pr-4 hover:bg-primary-200 hover:text-white">{{
            $childrens['nama'] }}</a></li>
        @endif
        @endforeach
