...
<link type='text/css' href="<?= assets('front/css/slider.css') ?>" rel='Stylesheet' />
<script src="<?= assets('front/js/jquery.cycle2.caption2.min.js') ?>"></script>
<style type="text/css">
		...
		<div class="content_middle_middle">
			<div id="aparatur_desa" class="cycle-slideshow" data-cycle-pause-on-hover=true data-cycle-fx=scrollHorz data-cycle-timeout=2000 data-cycle-caption-plugin=caption2 data-cycle-overlay-fx-out="slideUp" data-cycle-overlay-fx-in="slideDown" data-cycle-auto-height=4:6>
				<?php if ($this->web_widget_model->get_setting('aparatur_desa', 'overlay') == true) : ?>
					<span class="cycle-prev"><img src="<?= assets('images/back_button.png" alt="Back"></span>
					<span class="cycle-next"><img src="<?= assets('images/next_button.png" alt="Next"></span>
				...
				<?php foreach ($aparatur_desa['daftar_perangkat'] as $data) : ?>
					...
					?>
					<img data-src="<?= $data['foto'] ?>" src="<?= assets('images/img-loader.gif') ?>" class="yall_lazy" data-cycle-title="<?= $desc ?>" data-cycle-desc="<?= $data['jabatan'] ?>">
				<?php endforeach; ?>
			</div>
		</div>
	</div>
