...
(function() {
    ...
    const fromPoint = (docElm, x, y) =>
        Optional.from(docElm.dom.elementFromPoint(x, y)).map(fromDom$1);
    const SugarElement = {
        fromHtml,
        fromTag,
        fromText,
    ...
    const isComment = (element) =>
        type(element) === COMMENT || name(element) === "#comment";
    ...
    const children$3 = (element) =>
        map(element.dom.childNodes, SugarElement.fromDom);
    ...
    const isShadowRoot = (dos) =>
        isDocumentFragment(dos) && isNonNullable(dos.dom.host);
    const supported =
    ...
    const ancestor = (scope, selector, isRoot) =>
        ancestor$1(scope, (e) => is$2(e, selector), isRoot);
    ...
    const addPxSuffix = (size) =>
        /^\d+(\.\d+)?$/.test(size) ? size + "px" : size;
    const getSelectionStart = (editor) =>
        SugarElement.fromDom(editor.selection.getStart());
    const getSelectionEnd = (editor) =>
        SugarElement.fromDom(editor.selection.getEnd());
    const isInEditableContext = (cell) =>
        closest$2(cell, isTag("table")).forall(isEditable);
    ...
    const children$1 = (scope, selector) =>
        children$2(scope, (e) => is$2(e, selector));
    ...
    const is = (lhs, rhs, comparator = tripleEquals) =>
    ...
    const getAttrValue = (cell, name, fallback = 0) =>
        getOpt(cell, name)
    ...
    const columns = (ancestor) => {
        if (is$2(ancestor, "colgroup")) {
            ...
            return bind(columnGroups(ancestor), (columnGroup) =>
                children$1(columnGroup, "col"),
            );
        }
    };
    const table = (element, isRoot) => closest$1(element, "table", isRoot);
    const rows = (ancestor) => firstLayer(ancestor, "tr");
    const columnGroups = (ancestor) =>
        table(ancestor).fold(constant([]), (table) =>
            children$1(table, "colgroup"),
        );

    const fromRowsOrColGroups = (elems, getSection) =>
        map(elems, (row) => {
    ...
    const getLockedColumnsFromTable = (table) =>
        getOpt(table, LOCKED_COL_ATTR)
    ...
    const getAt = (warehouse, row, column) =>
    ...
    const generate$1 = (list) => {
        const access = {};
        const cells = [];
        const tableOpt = head(list)
            .map((rowData) => rowData.element)
            .bind(table);
        ...
        const {
            ...
            list,
            (rowData) => rowData.section === "colgroup",
        );
        each(rows, (rowData) => {
            const currentRow = [];
            each(rowData.cells, (rowCell) => {
                ...
                for (
                    let occupiedColumnPosition = 0; occupiedColumnPosition < rowCell.colspan; occupiedColumnPosition++
                ) {
                    for (
                        let occupiedRowPosition = 0; occupiedRowPosition < rowCell.rowspan; occupiedRowPosition++
                    ) {
    ...
    const getColumnAt = (warehouse, columnIndex) =>
    ...
    const getProp = (element, name, fallback) =>
        toNumber(get$1(element, name), fallback);
    ...
    const getCalculatedWidth = (element, boxSizing) => {
        ...
        return boxSizing === "border-box" ?
            width :
            calcContentBoxSize(element, width, "left", "right");
    };
    ...
    const getPixelForcedWidth = (editor) => {
        ...
        const parentBlock =
            (_a = dom.getParent(editor.selection.getStart(), dom.isBlock)) !== null &&
            _a !== void 0 ?
            _a :
            editor.getBody();
    ...
    const isResponsiveForced = (editor) =>
        getTableSizingMode(editor) === "responsive";
    const getDefaultStyles = (editor) => {
        const options = editor.options;
        const defaultStyles = options.get("table_default_styles");
        return options.isSet("table_default_styles") ?
            defaultStyles :
            determineDefaultStyles(editor, defaultStyles);
    };
    const getDefaultAttributes = (editor) => {
        const options = editor.options;
        const defaultAttributes = options.get("table_default_attributes");
        return options.isSet("table_default_attributes") ?
            defaultAttributes :
            determineDefaultAttributes(editor, defaultAttributes);
    };
    ...
    var DomUniverse = () => {
        ...
        const getLanguage = (element) =>
            isElement(element) ? getOpt(element, "lang") : Optional.none();
        return {
            up: constant({
                selector: ancestor,
                closest: closest$1,
        ...
        return getEdges(
            ...
            return firstAncestor.bind((fA) => {
                return lastAncestor.bind((lA) => {
                    return eq(fA, lA) ?
                        getBox(edges.table, edges.first, edges.last) :
                        Optional.none();
                });
            });
        });
    ...
    const getSelectionCellFallback = (element) =>
        table(element)
        .bind((table) => retrieve(table, ephemera.firstSelectedSelector))
        .fold(constant(element), (cells) => cells[0]);
    const getSelectionFromSelector = (selector) => (initCell, isRoot) => {
        const cellName = name(initCell);
        const cell =
            cellName === "col" || cellName === "colgroup" ?
            getSelectionCellFallback(initCell) :
            initCell;
        return closest$1(cell, selector, isRoot);
    };
    const getSelectionCellOrCaption = getSelectionFromSelector("th,td,caption");
    const getSelectionCell = getSelectionFromSelector("th,td");
    const getCellsFromSelection = (editor) =>
        fromDom(editor.model.table.getSelectedCells());
    const getRowsFromSelection = (selected, selector) => {
        const cellOpt = getSelectionCell(selected);
        const rowsOpt = cellOpt
            .bind((cell) => table(cell))
            .map((table) => rows(table));
        return lift2(cellOpt, rowsOpt, (cell, rows) =>
            filter(rows, (row) =>
                exists(
                    fromDom(row.dom.cells),
                    (rowCell) => get$2(rowCell, selector) === "1" || eq(rowCell, cell),
                ),
            ),
        ).getOr([]);
    };
        ...
        return (api) => {
            ...
            const init = () => {
                ...
                if (isNone) {
                    ...
                    boundCallback.set(
                        editor.formatter.formatChanged(formatName, api.setActive, false, {
                            value: formatValue,
                        }),
                    );
                }
    ...
    const buildMenuItems = (editor, items, format, onAction) =>
        map(items, (item) => {
            const text = item.text || item.title;
            if (isListGroup(item)) {
                return {
                    ...
                    text,
                    getSubmenuItems: () =>
                        buildMenuItems(editor, item.menu, format, onAction),
                };
            } else {
                return {
                    text,
    ...
    const generateMenuItemsCallback =
        (editor, items, format, onAction) => (callback) =>
    ...
    const getItems$2 = (editor) =>
        children.concat(getClassList$1(editor).toArray());
    ...
    const findInWarehouse = (warehouse, element) =>
        findMap(warehouse.all, (r) => find(r.cells, (e) => eq(element, e.element)));
    ...
    const onCells = (warehouse, target) =>
        extractCells(warehouse, target, always);
    const isUnlockedTableCell = (warehouse, cell) =>
        findInWarehouse(warehouse, cell).exists((detail) => !detail.isLocked);
    const allUnlocked = (warehouse, cells) =>
        forall(cells, (cell) => isUnlockedTableCell(warehouse, cell));
    const onUnlockedMergable = (warehouse, target) =>
        onMergable(warehouse, target).filter((mergeable) =>
            allUnlocked(warehouse, mergeable.cells),
        );
    const onUnlockedUnmergable = (warehouse, target) =>
        onUnmergable(warehouse, target).filter((cells) =>
            allUnlocked(warehouse, cells),
        );
    ...
    const rgbToHex = (value) =>
        startsWith(value, "rgb") ? rgbaToHexString(value) : value;
    ...
    const getAlignment = (formats, formatName, editor, elm) =>
        find(
            formats,
            (name) =>
            !isUndefined(editor.formatter.matchNode(elm, formatName + name)),
        ).getOr("");
    const getHAlignment = curry(
        getAlignment,
        ["left", "center", "right"],
        "align",
    );
    const getVAlignment = curry(
        getAlignment,
        ["top", "middle", "bottom"],
        "valign",
    );
    const extractDataFromSettings = (editor, hasAdvTableTab) => {
        ...
        const getBorder = () => {
            ...
            return get$4(attrs, "border").fold(
                () => ({}),
                (border) => ({
                    border
                }),
        ...
        const getCellPaddingCellSpacing = () => {
            const spacing = get$4(style, "border-spacing")
                .or(get$4(attrs, "cellspacing"))
                .fold(
                    () => ({}),
                    (cellspacing) => ({
                        cellspacing
            ...
            const padding = get$4(style, "border-padding")
                .or(get$4(attrs, "cellpadding"))
                .fold(
                    () => ({}),
                    (cellpadding) => ({
                        cellpadding
            ...
            table(cells[0]).each((table) => {
                ...
                const styleModified =
                    size(
                        filter$1(
                            modifiedData,
                            (_value, key) => key !== "scope" && key !== "celltype",
                        ),
                    ) > 0;
    ...
    const open$1 = (editor) => {
        ...
        const rowsData = map(rows, (rowElm) =>
            extractDataFromRowElement(editor, rowElm.dom, hasAdvancedRowTab(editor)),
        );
    ...
    const applyDataToElement = (editor, tableElm, data, shouldApplyOnCell) => {
        ...
        if (shouldStyleWithCss$1 && data.border > 0) {
        ...
        if (shouldStyleWithCss$1 && tableElm.children) {
            const cellStyles = {};
            if (shouldApplyOnCell.border && data.border > 0) {
            ...
            if (data.border == 0) {
                for (let i = 0; i < tableElm.children.length; i++) {
                    styleTDTH(dom, tableElm.children[i], "border-width", "");
                }
            }
        }
    ...
    const unmergable = (selectedCells) => {
        const hasSpan = (elem, type) =>
            getOpt(elem, type).exists((span) => parseInt(span, 10) > 1);
        const hasRowOrColSpan = (elem) =>
            hasSpan(elem, "rowspan") || hasSpan(elem, "colspan");
        return selectedCells.length > 0 && forall(selectedCells, hasRowOrColSpan) ?
            Optional.some(selectedCells) :
            Optional.none();
    };
    ...
    const getSelectionTargets = (editor) => {
        ...
        const isDisabledForSelection = (key) =>
            selectionDetails.forall((details) => !details[key]);
        const getStart = () =>
            getSelectionCellOrCaption(getSelectionStart(editor), getIsRoot(editor));
        const getEnd = () =>
            getSelectionCellOrCaption(getSelectionEnd(editor), getIsRoot(editor));
        const findTargets = () =>
            getStart().bind((startCellOrCaption) =>
                flatten(
                    lift2(
                        table(startCellOrCaption),
                        getEnd().bind(table),
                        (startTable, endTable) => {
        ...
        const onSetup = (api, isDisabled) =>
            setupHandler(() =>
                targets.get().fold(
                    ...
                    (targets) => {
                        api.setEnabled(
                            !isDisabled(targets) && editor.selection.isEditable(),
                        );
                    },
                ),
            );
        const onSetupWithToggle = (api, isDisabled, isActive) =>
            setupHandler(() =>
                targets.get().fold(
                    () => {
                        api.setEnabled(false);
        ...
        const isDisabledFromLocked = (lockedDisable) =>
        ...
        const onSetupCellOrRow = (api) =>
            onSetup(api, (targets) => isCaption(targets.element));
        const onSetupColumn = (lockedDisable) => (api) =>
            onSetup(
                api,
                (targets) =>
                isCaption(targets.element) || isDisabledFromLocked(lockedDisable),
            );
        const onSetupPasteable = (getClipboardData) => (api) =>
            onSetup(
                api,
                (targets) => isCaption(targets.element) || getClipboardData().isNone(),
            );
        const onSetupPasteableColumn = (getClipboardData, lockedDisable) => (api) =>
            onSetup(
                api,
                (targets) =>
                isCaption(targets.element) ||
                getClipboardData().isNone() ||
                isDisabledFromLocked(lockedDisable),
            );
        const onSetupMergeable = (api) =>
            onSetup(api, (_targets) => isDisabledForSelection("mergeable"));
        const onSetupUnmergeable = (api) =>
            onSetup(api, (_targets) => isDisabledForSelection("unmergeable"));
        ...
        const onSetupTableRowHeaders = onSetupTableHeaders(
            "mceTableRowType",
            "header",
        );
        const onSetupTableColumnHeaders = onSetupTableHeaders(
            "mceTableColType",
            "th",
        );
    ...
    const addButtons = (editor, selectionTargets) => {
        ...
        const addButtonIfRegistered = (name, spec) => {
            if (editor.queryCommandSupported(spec.command)) {
                editor.ui.registry.addButton(name, {
                    ...spec,
                    onAction: isFunction(spec.onAction) ?
                        spec.onAction :
                        cmd(spec.command),
                });
            }
        ...
        if (
            tableClassList.length !== 0 &&
            editor.queryCommandSupported("mceTableToggleClass")
        ) {
        ...
        if (
            tableCellClassList.length !== 0 &&
            editor.queryCommandSupported("mceTableCellToggleClass")
        ) {
            editor.ui.registry.addMenuButton("tablecellclass", {
                ...
                fetch: generateMenuItemsCallback(
                    ...
                    "tablecellclass",
                    (value) =>
                    editor.execCommand("mceTableCellToggleClass", false, value),
                ),
                onSetup: selectionTargets.onSetupCellOrRow,
            });
        }
            ...
            editor.ui.registry.addMenuButton("tablecellbackgroundcolor", {
                ...
                fetch: (callback) =>
                    callback(
                        buildColorMenu(
                            editor,
                            getTableBackgroundColorMap(editor),
                            "background-color",
                        ),
                    ),
                onSetup: selectionTargets.onSetupCellOrRow,
    ...
    const addToolbars = (editor) => {
        const isEditableTable = (table) =>
            editor.dom.is(table, "table") &&
            editor.getBody().contains(table) &&
            editor.dom.isEditable(table.parentNode);
    ...
    const addMenuItems = (editor, selectionTargets) => {
        ...
        if (
            ...
            editor.ui.registry.addNestedMenuItem("tableclass", {
                ...
                text: "Table styles",
                getSubmenuItems: () =>
                    buildMenuItems(editor, tableClassList, "tableclass", (value) =>
                        editor.execCommand("mceTableToggleClass", false, value),
                    ),
                onSetup: selectionTargets.onSetupTable,
            });
        }
